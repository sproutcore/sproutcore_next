...
/** @class
  ...
  If an View has a theme of "aki", it will look in its parent's theme
  for the theme "aki". If there is no parent--that is, if the view is a
  frame--it will look in Theme for the named theme. To find a theme,
  it calls find(themeName) on the theme.

  To be located, themes must be registered either as a root theme (by
  calling Theme.addTheme) or as a child theme of another theme (by
  ...
  by View when you name a theme that doesn't actually exist: it creates
...
export const Theme = {
    ...
    if (classNames) {
      if (SC.typeOf(classNames) === SC.T_HASH && !classNames.isSet) {
        ...
        if (typeof classNames === "string") {
          ...
          warn("Developer Warning: The classNames of a Theme should be an Array.");
          //@endif
  ...
  */
  ...
  /**
    Semi-private, only used by View to create "invisible" subthemes. You
    ...
    The reason for "invisible" subthemes is that View will create a subtheme
    ...
    If no "popup" theme can be found inside "base", View will call
    ...
    a child theme of "base"; View _needs_ to re-subtheme it, but it won't
  ...
  */
  find: function(themeName) {
    if (this === Theme) return this.themes[themeName];
    var theme;
...
export const BaseTheme = Theme.create({
...
Theme.themes['sc-base'] = BaseTheme;
SC.setSetting('defaultTheme', 'sc-base');
// export const defaultTheme = 'sc-base';
