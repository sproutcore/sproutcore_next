...
export const MODE_PREPEND = 'prepend';
...
*/
export const RenderContext = SC.Builder.create(
  /** @lends RenderContext */ {
  ...
  /**
    ...
    @returns {RenderContext} receiver
  */
  ...
  */
  ...
  html: function (line) {
    //@if(debug)
    SC.warn("Developer Warning: RenderContext:html() is no longer used to push HTML strings.  Please use `push()` instead.");
    //@endif
  ...
  */
  text: function () {
    ...
    for (idx = 0; idx < len; idx++) {
      this.push(RenderContext.escapeHTML(arguments[idx]));
    }
  ...
  */
  ...
  /**
    ...
    @returns {RenderContext} new context
  */
  begin: function (tagNameOrElement) {
    return RenderContext(tagNameOrElement, this);
  },
  ...
  */
  ...
  /**
    ...
    @returns {RenderContext} previous context or null if top
  */
  update: function () {
    var elem = this._elem,
  ...
  */
    ...
    if (attrs || className || styles || id) {
      ...
      if (styles) {
        for (key in styles) {
          ...
          if (typeof value === SC.T_NUMBER && !NON_PIXEL_PROPERTIES.contains(key)) value += "px";
    ...
    selfClosing = (this._selfClosing === false) ? false : (this.length === 1);
    tag = tag + (selfClosing ? ' />' : '>');
  ...
  /**
    ...
    @param {Object} opts optional tag options.  defaults to empty options.
    @returns {RenderContext} receiver
  */
  ...
  /**
    ...
    @returns {String|RenderContext} tag name or receiver
  */
  ...
  /**
    ...
    @returns {String|RenderContext} id or receiver
  */
  id: function (idName) {
    if (idName === undefined) {
  ...
  */
  ...
  /**
    ...
    This is a convenience method that simply calls setClass(nameOrClasses, true).
  ...
  addClass: function (nameOrClasses) {
    ...
    if (SC.typeOf(nameOrClasses) === SC.T_ARRAY) {
      for (var i = 0, length = nameOrClasses.length, obj = {}; i < length; i++) {
        obj[nameOrClasses[i]] = true;
      }
      nameOrClasses = obj;
    }

    return this.setClass(nameOrClasses, true);
  },

  /**
    Removes the specified class name from the current context.

    This is a convenience method that simply calls setClass(name, false).

    @param {String} name the class to remove
    @returns {RenderContext} receiver
  */
  removeClass: function (name) {
    return this.setClass(name, false);
  },
  ...
  /**
    ...
    @param {String|Object} nameOrClasses either a single class name or a hash of class names with boolean values indicating whether to add or remove the class
  ...
  */
  setClass: function (nameOrClasses, shouldAdd) {
    ...
    if (SC.typeOf(nameOrClasses) === SC.T_ARRAY) {
      //@if(debug)
      SC.warn("Developer Warning: RenderContext:setClass() should not be passed an array of class names.  To remain compatible with calls to the deprecated classNames() function, all classes on the current context will be replaced with the given array, but it would be more accurate in the future to call resetClasses() and addClass() or setClass(hash) instead.  Please update your code accordingly.");
      //@endif
      this.resetClasses();
      classes = this.classes();

      for (var i = 0, length = nameOrClasses.length; i < length; i++) {
        didChange = this._setClass(classes, nameOrClasses[i], true) || didChange;
      }
  ...
  */
  ...
  /**
    ...
    addClass('b') followed by setClass({ b:false }), the 'b' class will be
  ...
  */
  styles: function (deprecatedArg) {
    ...
    if (deprecatedArg) {
      //@if(debug)
      SC.warn("Developer Warning: RenderContext:styles() is no longer used to set styles, only to retrieve them.  Please use `setStyle(%@)` instead.".fmt(deprecatedArg));
      //@endif
  ...
  */
  addAttr: function (nameOrAttrs, value) {
    ...
    if (SC.typeOf(nameOrAttrs) === SC.T_STRING && value == null) {
      SC.warn("Developer Warning: RenderContext:addAttr is not meant to be used to remove attributes by setting the value to null or undefined.  It would be more correct to use setAttr(%@, %@).".fmt(nameOrAttrs, value));
    }
    //@endif
  ...
  */
  removeAttr: function (name) {
    ...
    // Notify when this function isn't being used properly (in debug mode only).
    if (name.match(/^(class|style)$/i)) {
      SC.error("Developer Error: RenderContext:removeAttr is not meant to be used to remove the style or class attribute.  You should use resetClasses() or resetStyles().");
    }
    //@endif
...
(function () {
  ...
  */
  RenderContext.escapeHTML = function (text) {
    if (!text) return '';
