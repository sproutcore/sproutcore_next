...
/**
  ...
  - *`supportsAutoResize`.* Must be set to true.
  ...
  rather than the user of AutoResize, will be providing the properties, and mixing
  AutoResize into the view should not override these properties.
*/
export const AutoResize = {
  /*@scope AutoResize.prototype */

  /**
    If true, automatically resizes the view (default). If false, only measures,
  ...
  */
  ...
  /**
    If false, prevents AutoResize from doing anything at all.
  ...
  */
  ...
  /**
    ...
    on calculation. Default is false to retain backwards
    compatibility.
  ...
  */
  ...
  /**
    ...
    shouldAutoResize to false, allows you to customize the 'sizing' part, using
    AutoResize purely for its measuring code.
  ...
  */
  ...
  /**
    If true, the view's text will be resized to fit the view. This is applied _after_ any
  ...
  */
  shouldAutoFitText: false,

  /**
    If false, the calculated font size may be any size between minFontSize and
    maxFontSize. If true, it will only be either minFontSize or maxFontSize.

    @type Boolean
    @default false
  */
  autoFitDiscreteFontSizes: false,
  ...
  */
  ...
  /**
    If shouldAutoFitText is true, this is the calculated font size.
  */
  ...
  */
  ...
  initMixin: function() {
    if (!this.get('supportsAutoResize')) {
      throw new Error("View `%@` does not support automatic resize. See documentation for AutoResize".fmt(this));
    }
  },
  // @endif
  ...
  */
  scheduleMeasurement: function() {
    ...
    AutoResizeManager.scheduleMeasurementForView(this, batchResizeId);
  ...
  _cachedMetrics: function(key, value) {
    ...
    var cacheSlot = cacheSlotFor(this.get('batchResizeId') || this),
  ...
  */
  measureSize: function(batch) {
    ...
    if(cachedMetrics) {
      ...
      metrics = measureString(autoResizeText, ignoreEscape);
    }

    else {
      this.prepareLayerForStringMeasurement(layer);

      metrics = metricsForString(autoResizeText, layer, this.get('classNames'), ignoreEscape);
    }
  ...
  */
  fitTextToFrame: function() {
    ...
    // otherwise we have to find the actual best font size
      ...
      metrics = metricsForString(value, layer, classNames, ignoreEscape);
  ...
  /**
    Extends renderSettingsToContext to add font size if shouldAutoFitText is true.
  */
  ...
  */
  ...
  /** @private
    ...
    TODO: consider making the measurement state a formal View state
  */
 ...
 *
 * This used to be part of AutoResize, but we shouldn't mix these
 * methods/properties into each view.
 */
export const AutoResizeManager = {
  /**
  ...
  */
  doBatchResize: function() {
    ...
    if (this !== AutoResizeManager) {
      return AutoResizeManager.doBatchResize();
    }
      ...
      for(i = len - 1; i >= 0; --i) {
        ...
        if(view.get('isVisibleInWindow') && view.get('shouldMeasureSize') && (layer = view.get('autoResizeLayer'))) {
          ...
          if(!SC.none(autoResizeText) && autoResizeText !== "" && !view.get('_cachedMetrics') && !prepared) {
            ...
            prepareStringMeasurement(layer, view.get('classNames'));
            prepared = true;
          }

          view.measureSize(true);
        }
