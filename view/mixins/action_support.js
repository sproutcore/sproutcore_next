...
/**
  ...
  *Note* Because searching the responder chain is slower, it is recommended to specify an actual
  ...
  For views implementing `ActionSupport`, the `sender` will always be `this`, which can be useful
  ...
  passed to `fireAction()` of the view. Here is a simple example to help illustrate this,
      ...
      // View with ActionSupport
      var view = View.create(ActionSupport, {
...
*/
export const ActionSupport =
/** @scope ActionSupport.prototype */ {

  //@if(debug)
  // Provide some debug-only developer warning support.
  initMixin: function () {
    if (this.actionContext !== null) {
      SC.warn("Developer Warning: The `actionContext` property of `ActionSupport` has been deprecated. Please pass the `context` argument to `fireAction()` directly.");
    }
  },
  //@endif
  ...
  */
  ...
  fireAction: function (actionOrContext) {
    ...
    // String argument and no action (above) property, assume action method name. Use argument with actionContext property.
      ...
      //@if(debug)
      // Provide some debug-only developer warning support.
      SC.warn("Developer Warning: The signature of `ActionSupport.prototype.fireAction` has changed. Please set the `action` property on your view and only pass an optional context argument to `fireAction`.");
      //@endif
