...
import { ALIGN_BOTTOM, ALIGN_CENTER, ALIGN_JUSTIFY, ALIGN_LEFT, ALIGN_MIDDLE, ALIGN_RIGHT } from "../system/constants.js";
import { LAYOUT_HORIZONTAL, LAYOUT_VERTICAL } from "../views/view/layout.js";
...
/**
  ...
  Child views with useAbsoluteLayout===true will be ignored in the layout process.
...
export const FlowedLayout = {
  ...
  */
  ...
  /**
    If true, flowing child views are allowed to wrap to new rows or columns.
  ...
  */
  ...
  /**
    ...
    for instance, when the view is the contentView for a ScrollView.
  ...
  */
  layoutDidChangeFor: function layoutDidChangeFor (c) {
    ...
    if (l.left && l.left !== cl.left) same = false;
    else if (l.top && l.top !== cl.top) same = false;
    else if (!c.get('fillWidth') && l.width && l.width !== cl.width) same = false;
    else if (!l.width && !c.get('fillWidth') && f.width !== c._scfl_lastFrame.width) same = false;
    else if (!c.get('fillHeight') && l.height && l.height !== cl.height) same = false;
    else if (!l.height && !c.get('fillHeight') && f.height !== c._scfl_lastFrame.height) same = false;
  ...
  */
  ...
  /**
    ...
    By default, if it has isVisible: false or useAbsoluteLayout: true, it will not be included.
  ...
  */
  flowSizeForChild: function(idx, view) {
    ...
    if (layoutDirection === LAYOUT_HORIZONTAL && view.get('fillHeight')) {
      calc.height = l.minHeight || 0;
    } else if (layoutDirection === LAYOUT_VERTICAL && view.get('fillWidth')) {
      calc.width = l.minWidth || 0;
    }
  ...
  _scfl_calculatedSizeDidChange: function() {
    if(this.get('autoResize')) {
      if (this.get('layoutDirection') == LAYOUT_VERTICAL) {
  ...
  */
  _scfl_createPlan: function() {
    ...
    var isVertical = (layoutDirection === LAYOUT_VERTICAL);
    ...
    var plan = {
      ...
      // the various methods.
      isVertical: layoutDirection === LAYOUT_VERTICAL,
      isHorizontal: layoutDirection === LAYOUT_HORIZONTAL,
  ...
  */
  _scfl_distributeChildrenIntoRow: function(children, startingAt, row) {
    ...
    for (idx = startingAt; idx < len; idx++) {
      ...
      // startsNewRow === true, and if the item cannot fit. Neither applies if there
  ...
  */
  _scfl_positionChildrenInRow: function(row) {
    var items = row.items, len = items.length, idx, item, position, rowSize = 0,
        spacerCount = 0, spacerSize, align = row.plan.align, shouldExpand = true,
        leftOver = 0, noMaxWidth = false;
    ...
    if (align === ALIGN_JUSTIFY) spacerCount += len - 1;
    ...
    //
    position = 0;
    if (spacerCount === 0 && (align === ALIGN_RIGHT || align === ALIGN_BOTTOM)) {
      position = row.plan.maximumRowLength - row.rowLength;
    } else if (spacerCount === 0 && (align === ALIGN_CENTER || align === ALIGN_MIDDLE)) {
    ...
    for (idx = 0; idx < len; idx++) {
      ...
      // laterally
      if(!item.fillRow) shouldExpand = false;
      ...
      if (align === ALIGN_JUSTIFY && idx < len - 1) position += spacerSize;
    }

    row.shouldExpand = len > 0 ? shouldExpand : false;
