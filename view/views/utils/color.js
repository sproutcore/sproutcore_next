...
/** @class
  ...
  For instructions on using Color to color a view, see "Color and View"
  below.
  ...
  valid CSS string, with:
      ...
      Color.from('rgba(255, 255, 255, 0.5)');
  ...
  `Color.from` with an undefined or null value is equivalent to calling it with
  `'transparent'`.)
  ...
  `Color` provides three methods for performing basic math: `sub` for subtraction,
  ...
  for operations such as interpolating between colors (see "Color and View"
  below); it also gives Color more predictable math, where A + B - B = A, even if
  the intermediate (A + B) operation results in underlying values outside of the normal
  gamut.

  (The class method `Color.clampToDeviceGamut` is used to clamp r, g and b values to the
  ...
  Hooking up an instance of Color to View#backgroundColor is simple, but like all
  ...
  Use the following code to tie a view's background color to an instance of Color. Note
  ...
  when the it changes; for performance reasons it is not included by default.
      ...
      View.extend({
        color: Color.from({'burlywood'}),
        backgroundColorBinding: Binding.oneWay('*color.cssText'),
  ...
  more performant):
      ...
      View.extend({
        ...
        fromColor: Color.from('cadetblue'),
        toColor: Color.from('springgreen'),
        click: function() {
  ...
  If you call `Color.from` with an invalid value, or set `cssText` to an invalid
  value, the color object will go into error mode, with `isError` set to true and
  ...
  this case and provide the correct CSS snippet:
      ...
      var color = Color.from('rgba(0, 0, 0, .5)').get('cssText'),
          css;
      if (Color.supportsARGB) {
 ...
 */
export const Color = SC.Object.extend(
  SC.Copyable,
  ...
  /**
    ...
    For example, if your color was created via `Color.from("burlywood")`,
  ...
  */
  ...
  hue: function (key, deg) {
    var clamp = Color.clampToDeviceGamut,
        hsl = Color.rgbToHsl(clamp(this.get('r')),
                                clamp(this.get('g')),
                                clamp(this.get('b'))),
        rgb;
  ...
  /**
    ...
    @param {Color} color The color to compare this one to.
    @returns {Boolean} true if the two colors are equivalent
   */
  ...
  cssText: function (key, value) {
    ...
    if (value === undefined) {
      ...
      var supportsAlphaChannel = Color.supportsRgba ||
                                 Color.supportsArgb;
    ...
    // Setter.
      ...
      // Happy state
        ...
        this.setIfChanged('isError', false);
  ...
  /**
    ...
    @param {Color} color The color to subtract from this one.
    @returns {Color} The difference between the two colors.
   */
  sub: function (color) {
    return Color.create({
  ...
  /**
    ...
    @param {Color} color The color to add to this one.
    @returns {Color} The addition of the two colors.
   */
  add: function (color) {
    return Color.create({
      r: this.get('r') + color.get('r'),
...
Color.mixin(
  ...
  create: function() {
    ...
    args = new Array(arguments.length); // A(arguments)
    len = args.length;
  ...
  clampInt: function (value, min, max) {
    return Math.round(Color.clamp(value, min, max));
  },
  ...
  clampToDeviceGamut: function (value) {
    return Color.clampInt(value, 0, 255);
  },
  ...
  hsvToRgb: function (h, s, v) {
    ...
    var r, g, b,
        ...
        rgb = [[v, t, p],
               ...
               [t, p, v],
               [v, p, q]],
        clamp = Color.clampToDeviceGamut;
  ...
  /**
    Parses any valid CSS color into a `Color` object. Given invalid input, will return a
    `Color` object in an error state (with isError: true).

    @param {String} color The CSS color value to parse.
    @returns {Color} The color object representing the color passed in.
   */
  from: function (color) {
    // Fast path: clone another color.
    if (SC.kindOf(color, Color)) {
      return color.copy();
    }

    // Slow path: string
    var hash = Color._parse(color),
        C = Color;
  ...
  /** @private
    ...
    For internal use only. External code should call `Color.from` or `Color#cssText`.
