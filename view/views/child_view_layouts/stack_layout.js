...
_SC_VIEW_STACK_PLUGIN.prototype.layoutChildViews = function (view) {
          ...
          // Add some developer support.
            ...
            if (SC.none(layout[minSizeDimension]))
            {
              if (this.direction === 'vertical') {
                SC.warn('Developer Warning: The View.VERTICAL_STACK plugin requires that each childView layout contains at least a height or has a configured fillRatio. The layout may also optionally contain left and right, left and width, right and width or centerX and width. The childView %@ has an invalid layout/fillRatio: %@'.fmt(childView, stringFromLayout(layout)));
              } else {
                SC.warn('Developer Warning: The View.HORIZONTAL_STACK plugin requires that each childView layout contains at least a width or has a configured fillRatio. The layout may also optionally contain top and bottom, top and height, bottom and height or centerY and height. The childView %@ has an invalid layout/fillRatio: %@'.fmt(childView, stringFromLayout(layout)));
              }
              return;
            }
          }
  ...
  for (i = 0, len = childViews.get('length'); i < len; i++) {
    ...
    if (resizeToFit && SC.none(layout[sizeDimension]) && SC.none(layout[minSizeDimension])) {
      if (this.direction === 'vertical') {
        SC.warn('Developer Warning: The View.VERTICAL_STACK plugin, when configured with resizeToFit, requires that each childView layout contains at least a height/minHeight and optionally also left and right, left and width, right and width or centerX and width.  The childView %@ has an invalid layout: %@'.fmt(childView, stringFromLayout(layout)));
      } else {
        SC.warn('Developer Warning: The View.HORIZONTAL_STACK plugin, when configured with resizeToFit, requires that each childView layout contains at least a width/minWidth and optionally also top and bottom, top and height, bottom and height or centerY and height.  The childView %@ has an invalid layout: %@'.fmt(childView, stringFromLayout(layout)));
      }
      return;
    }
    //@endif
...
export const staticStackLayoutMixin = /** @scope View */ {
  ...
  /**
    ...
    @extends ChildViewLayoutProtocol
    @since Version 1.10
  */
