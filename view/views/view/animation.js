...
/**
  ...
  For example,
      ...
      // later..
      myView.cancelAnimation(LayoutState.START);
  ...
  For example,
      ...
      // later..
      myView.cancelAnimation(LayoutState.CURRENT);
  ...
  For example,
      ...
      // later..
      myView.cancelAnimation(LayoutState.END);
...
View.reopen(
  ...
  /**
    ...
    The method you provide to View.prototype.animate should accept the
    ...
    @param {View} animationResult.view The animated view.
  */
  ...
  */
  animate: function (key, value, options, target, method) {
    ...
    for (var property in hash) {
      // Fast path.
      if (!hash.hasOwnProperty(property) || !ANIMATABLE_PROPERTIES[property]) {
      ...
      if (curAnim && (curAnim.duration !== options.duration ||
        ...
        this.runAnimationCallback(curAnim, null, true);
      }
  ...
  */
  ...
  /**
    ...
    If no `layoutState` is given or if LayoutState.END is given, the view
    will be adjusted to its final layout.  If LayoutState.START is given,
    the view will be adjusted back to its initial layout and if
    LayoutState.CURRENT is given, the view will stop at its current layout
    ...
    isCancelled property set to true.

    @param {LayoutState} [layoutState=LayoutState.END] The layout to immediately adjust the view to.
    @returns {View} this
  */
  cancelAnimation: function (layoutState) {
    ...
    for (key in pendingAnimations) {
      ...
      // Run the callback.
      this.runAnimationCallback(animation, null, true);
    }
    ...
    for (key in activeAnimations) {
      ...
      this.removeAnimationFromLayout(key, true);
  ...
  */
  didRenderAnimations: function () {
    ...
    if (!platform.supportsCSSTransitions || document.hidden) {
      var pendingAnimations = this._pendingAnimations;

      for (var key in pendingAnimations) {
        this.removeAnimationFromLayout(key, false);
        this.runAnimationCallback(pendingAnimations[key], null, false);
      }
  ...
  _sc_decompose3DTransformMatrix: function (matrix, expectsScale) {
    ...
    // Find the 3 Euler angles. Note the order applied using CSS_TRANSFORM_NAMES in layout_style.js.
  ...
  */
  liveAdjustments: function () {
    var activeAnimations = this._activeAnimations,
      el = this.get('layer'),
      ret = {},
      transformKey = browser.experimentalCSSNameFor('transform');
  ...
  removeAnimationFromLayout: function (propertyName, shouldUpdateStyle) {
    ...
    if (!!layer && shouldUpdateStyle) {
      ...
      layer.style[browser.experimentalStyleNameFor('transition')] = updatedCSS.join(', ');
    }
  },
  ...
  */
  transitionDidEnd: function (evt) {
    ...
    if (animation) {
      ...
      // Remove the animation style without triggering a layout change.
      this.removeAnimationFromLayout(propertyName, true);
      ...
      // Run the callback.
      this.runAnimationCallback(animation, evt, false);
    }
  },
    ...
    if (platform.supportsCSSTransitions && !document.hidden) {
      ...
      if (pendingAnimations) {
        ...
        for (var key in pendingAnimations) {
          ...
          if (activeAnimation) {
            this.runAnimationCallback(activeAnimation, null, true);
          }
