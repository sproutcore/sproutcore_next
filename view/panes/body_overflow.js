...
/**
  bodyOverflowArbitrator is a central object responsible for controlling the overflow on the body element.
...
*/
export const bodyOverflowArbitrator = SC.Object.create(/**@scope bodyOverflowArbitrator.prototype */{
  ...
  _decideOverflow: function () {
    ...
    for (var i in reqs) {
      if ((req = reqs[i]) < 0) haveHidden = true;
      if (req < -1) haveImportantHidden = true;
      if (req > 0) haveVisible = true;
      if (req > 1) haveImportantVisible = true;
    }
    if (haveImportantVisible) return true;              //important-visible takes all.
    if (haveVisible && haveImportantHidden) return false; //important-hidden beats regular-visible.
    if (haveVisible) return true;                       //regular-visible beats regular-hidden
    if (haveHidden) return false;                         //if there is a hidden, it can win now.
