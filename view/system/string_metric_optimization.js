...
/**
  This function is similar to metricsForString, but takes an extra argument after the string and before the exampleElement.
  ...
  result of all the following values:
    ...
    - Whether ignoreEscape is true or false
  ...
  instead of an element, or unless your element has *cacheableForMetrics: true* as a key on it. In most situations, the styles on
  an element won't change from call to call, so this is purely defensive and for arguably infrequent benefit, but it's good
  insurance. If you set the *cacheableForMetrics* key to true on your exampleElement, caching will kick in, and repeated calls to
  ...
  @param {Element/String} exampleElement The element whose styles will be used to measure the width and height of the string.
                                          You can pass a string of CSSText here if you wish, just as with metricsForString.
  ...
  @param {Boolean} [ignoreEscape] Optional. If true, HTML in your string will not be escaped. If false or omitted, any HTML
                                            ...
                                            otherwise required.
*/
export const bestStringMetricsForMaxWidth = function me (string,maxWidth,exampleElement,classNames,ignoreEscape) {
  if(!maxWidth) { warn("When calling bestMetricsForWidth, the second argument, maxWidth, is required. There's no reason to call this without a maxWidth."); return undefined; }
  maxWidth = Math.ceil(parseFloat(maxWidth));
  var       exIsElement = SC.typeOf(exampleElement||(exampleElement=""))!==SC.T_STRING,
          savedMaxWidth = exIsElement ? exampleElement.style.maxWidth : undefined,
                  cache = (!exIsElement || exampleElement.cacheableForMetrics) ?
                            cacheSlotFor(exampleElement,classNames,ignoreEscape,string) :
                            undefined,
  ...
  var            exEl = applyMax(exampleElement,maxWidth),
              metrics = metricsForString(string,exEl,classNames,ignoreEscape),
      necessaryHeight = metrics.height,
        oneLineHeight = cache ? cache.parent.height || (cache.parent.height=metricsForString('W',exEl,classNames).height) : metricsForString('W',exEl,classNames).height,
  ...
  if(necessaryHeight > oneLineHeight) {
    ...
    if(searchingUpward) metrics = metricsForString(string,exEl=removeMax(exEl),classNames,ignoreEscape);
    metrics.maxWidth = maxWidth;
    metrics.lineHeight = oneLineHeight;
    metrics.lines = lines;
...
/**
  ...
  @returns {Object} A cached workspace mapped to the ordered *n*-tuple of arguments passed into it.
*/
export const cacheSlotFor = function() {
