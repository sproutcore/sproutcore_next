...
SC.mixin(SCEvent, /** @scope SCEvent */ {
  ...
  */
  _MOUSE_WHEEL_LIMIT_INVALIDATED: false,
  ...
  */
  add: function(elem, eventType, target, method, context, useCapture) {
    ...
    // eventType.
    var events = CoreQuery.data(elem, "sc_events") || CoreQuery.data(elem, "sc_events", {}),
  ...
  */
  ...
  /**
    ...
    @param {Object} attrs optional additional attributes to apply to event.
  ...
  */
  handle: function (event) {
    ...
    handlers = (CoreQuery.data(this, "sc_events") || {})[event.type];
  ...
  */
  _addEventListener: function(elem, eventType, useCapture) {
    ...
    if ( !special || special.setup.call(elem) === false) {
      ...
      listener = CoreQuery.data(elem, "listener") || CoreQuery.data(elem, "listener",
  ...
  */
  _removeEventListener: function(elem, eventType) {
    ...
    if (!special || (special.teardown.call(elem)===false)) {
      ...
      listener = CoreQuery.data(elem, "listener") ;
      if (listener) {
  ...
  */
  commandCodes: function () {
    ...
    if (keyCode && keyCode !== charCode) {
      commandCode = FUNCTION_KEYS[keyCode];
    }
    ...
    // Find the base character for the key (i.e. `alt` + `a` becomes `å`, but we really want the key name, `a`).
      ...
      baseKeyName = PRINTABLE_KEYS[keyCode];
    }
