...
/**
  ...
  The following example shows how states are nested within a statechart:
      ...
      MyApp.Statechart = SC.Object.extend(StatechartManager, {
...
export const StatechartManager = /** @scope StatechartManager.prototype */{
  ...
  /**
    ...
    falseTE: This is only available in debug mode!
    ...
    if (this._gotoStateActions) {
      ...
      var actions = this._gotoStateActions,
          actionToStr = function (action) {
            var actionName = action.action === ENTER_STATE ? "enter" : "exit";
  ...
  */
  initStatechart: function () {
    ...
    if (this.get('monitorIsActive')) {
      this.set('monitor', StatechartMonitor.create({ statechart: this }));
    }
    ...
    if (trace) this.statechartLogTrace("BEGIN initialize statechart", TRACE_STATECHART_STYLE.init);
    //@endif
    ...
    if (!(SC.kindOf(rootState, State) && rootState.isClass)) {
    ...
    if (SC.kindOf(rootState.get('initialSubstate'), EmptyState)) {
    ...
    //@if(debug)
    if (trace) this.statechartLogTrace("END initialize statechart", TRACE_STATECHART_STYLE.init);
    //@endif
  },
  ...
  */
  ...
  /**
    ...
    @param context {Object} Optional. A context object that will be passed to all exited and entered states
  */
  gotoState: function (state, fromCurrentState, useHistory, context) {
    ...
    var args = new Array(arguments.length); //  $A(arguments)
    ...
    if (trace) {
      this.statechartLogTrace("BEGIN gotoState: %@".fmt(state), TRACE_STATECHART_STYLE.gotoState);
      msg = "  starting from current state: %@";
      msg = msg.fmt(fromCurrentState ? fromCurrentState : '-- none --');
      this.statechartLogTrace(msg, TRACE_STATECHART_STYLE.gotoStateInfo);
    ...
    if (pivotState) {
      //@if(debug)
      if (trace) this.statechartLogTrace("  pivot state = %@".fmt(pivotState), TRACE_STATECHART_STYLE.gotoStateInfo);
      //@endif
  ...
  */
  ...
  _executeGotoStateActions: function (gotoState, actions, marker, context) {
    ...
    for (; marker < len; marker += 1) {
      ...
      //
      if (SC.kindOf(actionResult, Async)) {
    ...
    if (this.get('allowStatechartTracing')) {
      if (this.getPath('currentStates.length') > 2) {
        this.statechartLogTrace("  current states after:\n%@".fmt(this.get('currentStates').getEach('fullPath').join('  \n')), TRACE_STATECHART_STYLE.gotoStateInfo);
      } else {
        this.statechartLogTrace("  current states after: %@".fmt(this.get('currentStates').getEach('fullPath').join(', ')), TRACE_STATECHART_STYLE.gotoStateInfo);
      }
      this.statechartLogTrace("END gotoState: %@".fmt(gotoState), TRACE_STATECHART_STYLE.gotoState);
    }
    //@endif
  ...
  _exitState: function (state, context) {
    ...
    //@if(debug)
    if (this.get('allowStatechartTracing')) {
      this.statechartLogTrace("<-- exiting state: %@".fmt(state), TRACE_STATECHART_STYLE.exit);
    }
    //@endif
  ...
  /**
    ...
    @param context {Object} a context hash object to provide the enterState method
  */
  ...
  _enterState: function (state, current, context) {
    ...
    if (this.get('allowStatechartTracing')) {
      if (state.enterStateByRoute && SC.kindOf(context, StateRouteHandlerContext)) {
        this.statechartLogTrace("--> entering state (by route): %@".fmt(state), TRACE_STATECHART_STYLE.enter);
      } else {
        this.statechartLogTrace("--> entering state: %@".fmt(state), TRACE_STATECHART_STYLE.enter);
      }
    }
    //@endif
  ...
  */
  sendEvent: function (event, arg1, arg2) {
    ...
    var trace = this.get('allowStatechartTracing');
    if (trace) {
      this.statechartLogTrace("BEGIN sendEvent: '%@'".fmt(event), TRACE_STATECHART_STYLE.action);
    }
    //@endif
    ...
    //@if(debug)
    if (trace) {
      if (!statechartHandledEvent) this.statechartLogTrace("No state was able to handle event %@".fmt(event), TRACE_STATECHART_STYLE.action);
      this.statechartLogTrace("END sendEvent: '%@'".fmt(event), TRACE_STATECHART_STYLE.action);
    }
    //@endif
  ...
  */
  _traverseStatesToExit: function (state, exitStatePath, stopState, gotoStateActions) {
    ...
    gotoStateActions.push({ action: EXIT_STATE, state: state });
  ...
  */
  _traverseStatesToEnter: function (state, enterStatePath, pivotState, useHistory, gotoStateActions) {
    ...
    // other criteria
      ...
      var gotoStateAction = { action: ENTER_STATE, state: state, currentState: false };
      ...
      // State has an initial substate to enter
        ...
        if (SC.kindOf(initialSubstate, HistoryState)) {
    ...
    // Still have an explicit enter path to follow, so keep moving through the path.
      ...
      gotoStateActions.push({ action: ENTER_STATE, state: state });
  ...
  */
  ...
  _monitorIsActiveDidChange: function () {
    if (this.get('monitorIsActive') && SC.none(this.get('monitor'))) {
      this.set('monitor', StatechartMonitor.create());
    }
  ...
  */
  _constructRootStateClass: function () {
    ...
    if (!(SC.kindOf(rsExample, State) && rsExample.isClass)) {
    ...
    for (key in this) {
      ...
      if (SC.kindOf(value, State) && value.isClass && this[key] !== this.constructor) {
        attrs[key] = value;
        stateCount += 1;
      }
    }
...
SC.mixin(StatechartManager, StatechartDelegate, SC.DelegateSupport);

/**
  A Statechart class.
*/
export const Statechart = SC.Object.extend(StatechartManager, {
  autoInitStatechart: false
});

Statechart.design = Statechart.extend;
