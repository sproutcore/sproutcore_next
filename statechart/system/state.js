...
*/
export const State = SC.Object.extend(
  ...
  */
  initState: function() {
    ...
    if (SC.kindOf(initialSubstate, HistoryState) && initialSubstate.isClass) {
  ...
  */
  routeTriggered: function(params) {
    ...
    if (del.statechartShouldStateHandleTriggeredRoute(sc, this, context)) {
      ...
      if (this.get('trace') && loc) {
        this.stateLogTrace("will handle route '%@'".fmt(loc), TRACE_STATECHART_STYLE.route);
      }
      //@endif
  ...
  /**
    ...
    @param {Object} attr attributes to apply to the constructed object
  ...
  */
  ...
  _addEmptyInitialSubstateIfNeeded: function() {
    ...
    var state = this.createSubstate(EmptyState);
    this.set('initialSubstate', state);
  ...
  */
  addSubstate: function(name, state, attr) {
    ...
    var stateIsValid = SC.kindOf(state, State) && state.isClass;
  ...
  */
  getSubstate: function(value, callback, target) {
    ...
    var matcher = StatePathMatcher.create({ state: this, expression: value }),
        matches = [], key;
  ...
  */
  getState: function(value) {
    ...
    if (SC.kindOf(value, State)) return value;
  ...
  /**
    ...
    @param [context] {Object} context object that will be supplied to all states that are
  ...
  */
  ...
  /**
    ...
    Method can be called in the following ways:
        ...
        // With two arguments
        gotoHistoryState(<value>, <boolean | object>)

        // With three arguments
        gotoHistoryState(<value>, <boolean>, <object>)

    Where <value> is either a string or a State object and <object> is a regular JS object object.
    ...
    @param [context] {Object} context object that will be supplied to all states that are exited
  ...
  */
  tryToHandleEvent: function(event, arg1, arg2) {
    ...
    if (SC.typeOf(this[event]) === SC.T_FUNCTION) {
      //@if(debug)
      if (trace) this.stateLogTrace("will handle event '%@'".fmt(event), TRACE_STATECHART_STYLE.actionInfo);
      //@endif
    ...
    if (handler) {
      //@if(debug)
      if (trace) this.stateLogTrace("%@ will handle event '%@'".fmt(handler.name, event), TRACE_STATECHART_STYLE.actionInfo);
      //@endif
    ...
    if (SC.typeOf(this['unknownEvent']) === SC.T_FUNCTION) {
      //@if(debug)
      if (trace) this.stateLogTrace("unknownEvent will handle event '%@'".fmt(event), TRACE_STATECHART_STYLE.actionInfo);
      //@endif
  ...
  /**
    ...
    @param {Object} [context] value if one was supplied to gotoState when invoked
  ...
  */
  ...
  /**
    ...
    @param context {Object} Optional value if one was supplied to gotoState when invoked
    @see #enterState
  */
    ...
    for (key in this._registeredStateObserveHandlers) {
      ...
      for (i = 0; i < values.length; i += 1) {
        ...
        if (dotIndex < 0) {
          ...
          SC.ObserverQueue[action](path, this, observer, root);
        }
      }
    }
  },
  ...
  */
  performAsync: function(func, arg1, arg2) {
    return Async.perform(func, arg1, arg2);
  },
...
/**
  ...
  @param args {Object,...} Optional. Object objects to be added to the created state
*/
State.plugin = function(value) {
  var args;
  ...
  args = new Array(arguments.length - 1); // A(arguments).shift()
  for (var i = 0, len = args.length; i < len; i++) { args[i] = arguments[i + 1]; }

  var func = function() {
    var klass = SC.objectForPropertyPath(value);
    if (!klass) {
      console.error('State.plugin: Unable to determine path %@'.fmt(value));
      return undefined;
    }
    if (!klass.isClass || !klass.kindOf(State)) {
      console.error('State.plugin: Unable to extend. %@ must be a class extending from State'.fmt(value));
      return undefined;
    }
