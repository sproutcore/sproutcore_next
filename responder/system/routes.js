...
/**
  ...
  routes manages the browser location. You can change the hash part of the
  current location. The following code

      routes.set('location', 'notes/edit/4');
  ...
  changes and matches the route:
      ...
      routes.add(':controller/:action/:id', MyApp, MyApp.route);

  You can pass additional parameters in the location hash that will be relayed
  to the route handler:

      routes.set('location', 'notes/show/4?format=xml&language=fr');
  ...
  presses the 'back' or 'forward' button, the location is changed, route
  ...
  routes also supports HTML5 history, which uses a '/' instead of a '#'
  in the URLs, so that all your website's URLs are consistent.
*/
export const routes = SC.Object.create(
  /** @scope routes.prototype */{
  ...
  */
  ping: function() {
    ...
    if (!this._didSetup) {
      ...
      if (this.get('wantsHistory') && platform.supportsHistory) {
        this.usesHistory = true;

        this.popState();
        SCEvent.add(window, 'popstate', this, this.popState);

      } else {
        this.usesHistory = false;

        if (platform.supportsHashChange) {
          this.hashChange();
          SCEvent.add(window, 'hashchange', this, this.hashChange);

        } else {
          // we don't use a Timer because we don't want
  ...
  */
  ...
  /**
    ...
    @param {Boolean} coerce coerce the values? (Default false)
  */
  ...
  /**
    ...
    routes._Route is a class used internally by routes. The routes defined by your
    application are stored in a tree structure, and this is the class for the
    nodes.
  */
  _Route: SC.Object.extend(
  /** @scope routes._Route.prototype */ {
