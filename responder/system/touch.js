...
/** @class Touch
  ...
  Touch exposes a number of properties, including pageX/Y, clientX/Y, screenX/Y, and startX/Y (the values that
  pageX/Y had when the touch began, useful for calculating how far the touch has moved). It also exposes the touch's
  target element at `target`, its target View at `targetView`, and the touch's unique identifier at `identifier`,
  ...
  - `captureTouch` -- Sometimes, a touch responder part way up the chain may need to capture the touch and prevent it
    from being made available to its childViews. The canonical use case for this behavior is ScrollView, which by
    default captures touches and holds onto them for 150ms to see if the user is scrolling, only passing them on to
    children if not. (See ScrollView#delaysContentTouches for more.) In order to support this use case, `captureTouch`
    bubbles the opposite way as usual: beginning with the target's pane and bubbling *down* towards the target itself.
    `captureTouch` is passed a single instance of `Touch`, and must return true if it wishes to capture the touch and
  ...
  at a time, though a view with `acceptsMultitouch: true` may respond to more than one touch at a time.

  A view becomes a touch responder by implementing touchStart (and not returning false). (Out-of-order views can capture
  touch responder status by implementing captureTouch and returning true.) Once a view is a touch responder, only that
  ...
  `touchCancelled` events, for example as an alternative to returning true from `captureTouch`, you can call
  ...
  again.
*/
export const Touch = function(touch, touchContext) {
  ...
  if (target && CoreQuery(target).hasClass("touch-intercept")) {
    ...
    if (target.tagName === "INPUT") {
      ...
      touch.target.style[browser.experimentalStyleNameFor('transform')] = "translate3d(0px,0px,0px)";
    }
  } else {
    targetView = touch.target ? viewFor(touch.target) : null;
  }
...
Touch.prototype = {
  ...
  // Debug-mode only.
  toString: function () {
    return "Touch (%@)".fmt(this.identifier);
  },
  //@endif

  /**@scope Touch.prototype*/
  ...
  */
  ...
  unhideTouchIntercept: function() {
    ...
    if (intercept) {
      setTimeout(function() { intercept.style[browser.experimentalStyleNameFor('transform')] = "translate3d(0px,0px,0px)"; }, 500);
    }
  },
  ...
  */
  ...
  /**
    ...
    @param {Responder} view The view which should become this touch's new responder.
  ...
  */
  stackNextTouchResponder: function(view, upStack) {
    this.makeTouchResponder(view, true, upStack);
  },
  ...
  */
  ...
  /**
    Changes the touch responder for the touch. If shouldStack is true,
    ...
    method which is passed a touch object and returns true. This method
  ...
  */
  ...
  /**
    ...
    having touchStart etc. called if it doesn't end up coming into play. For example, ScrollView
    adds itself as a candidate responder (when delaysTouchResponder is false) so that views can easily
  ...
  */
  ...
  /**
    ...
    touch's target view to find a view which implements `captureTouch` and returns true. If the touch
    is captured, then this method will perform a standard `touchStart` event bubbling beginning with
    the view which captured the touch. If no view captures the touch, then this method returns false,
    ...
    @param {?Responder} startingPoint The view whose children should be given an opportunity
  ...
  */
  ...
  /**
    A synonym for Touch#touchesForView.
  */
  ...
  /**
    ...
    @param {Responder} view
    @param {Boolean} addSelf Includes the receiver in calculations. Pass true for this if calling
...
SC.mixin(Touch, {
  ...
  create: function(touch, touchContext) {
    return new Touch(touch, touchContext);
  },
