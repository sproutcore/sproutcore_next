...
/**
  ...
  Request is essentially a client version of the request/response objects you receive when
  ...
  and `type` of the request must be set on every request, there are helper methods on `Request`
  ...
  By default, the request will create an instance of `XHRResponse` in order to complete the
  request. As the name implies, the `XHRResponse` class will make an XHR request to the server,
  which is the typical method that the SproutCore client will communicate with remote endpoints.

  In order to use a custom response type handler, you should extend `Response` and set the
  `responseClass` of the request to your custom response type.

  For example,

      var request = Request.getUrl(resourceAddress).set('responseClass', MyApp.CustomProtocolResponse);


  ### Handling Responses

  `Request` supports multiple response handlers based on the status code of the response. This
  ...
  so,
      ...
      // The response handler target (typically a state or controller or some such Object instance).
      var targetObject;
      ...
      request = Request.getUrl(resourceAddress)
  ...
  For example, here is a more specific example,
      ...
      // The response handler target (typically a data source or state or some such Object instance).
      var targetObject;
  ...
  For more examples, including handling of XHR2 progress events, please @see Request.prototype.notify.
...
*/
export const Request = SC.Object.extend(SC.Copyable, SC.Freezable,
/** @scope Request.prototype */ {
  ...
  */
  ...
  /**
    ...
    to it. By default, Request attaches X-Requested-With and
    X-SproutCore-Version headers to all outgoing requests. This allows
    you to override that behavior.

    You may want to set this to false if you are making simple CORS requests
  ...
  */
  ...
  /**
    ...
    only supported option is XHRResponse which uses a traditional
    XHR transport.

    @type Response
    @default XHRResponse
  */
  responseClass: XHRResponse,
  ...
  */
  ...
  /**
    ...
    begins when Response#fire is actually invoked by the request manager
    and not necessarily when Request#send is invoked. If this timeout is
    reached before a response is received, the equivalent of
    Request.manager#cancel() will be invoked on the Response instance
  ...
  */
  ...
  /**
    ...
    @param {Request} request A copy of the request object, not frozen
    @param {Response} response The object that will wrap the response
  */
  ...
  /**
    ...
    @param {Request} request A copy of the request object, frozen
    @param {Response} response The object that will wrap the response
  ...
  */
  ...
  /**
    ...
    @param {String} value
    @returns {Request|Object} receiver
  */
  ...
  */
  ...
  /**
    ...
    @param {Boolean} flag true to make asynchronous, false or undefined. Default true.
    @returns {Request} receiver
  */
  async: function(flag) {
  ...
  /**
    ...
    @param {Boolean} flag true to request allowing credentials, false to disallow credentials. Default true.
    @returns {Request} receiver
  */
  ...
  */
  ...
  /**
    ...
    @param {Boolean} flag true to make JSON, false or undefined. Default true.
    @returns {Request} receiver
  */
  json: function(flag) {
    if (flag === undefined) { flag = true; }
  ...
  /**
    ...
    @param {Boolean} flag true to make XML, false or undefined. Default true.
    @returns {Request} recevier
  */
  xml: function(flag) {
    if (flag === undefined) { flag = true; }
  ...
  */
  ...
  /**
    ...
    @returns {Response} New response object
  */
  send: function(body) {
    ...
    return Request.manager.sendRequest(this.copy()._prep());
  },
  ...
  */
  resend: function() {
    ...
    return Request.manager.sendRequest(req);
  },
  ...
  /**
    ...
    Request.CREATED), the accompanying method will only be called if the
    ...
    specific notifier was not registered first and returned true.
    ...
    more specific notifier was registered first and returned true.

    For example,

        var req = Request.create({type: 'POST'});
    ...
    Request responseClass, XHRResponse, you can be notified of each of
    ...
    For example,
      ...
      var req = Request.create({type: 'GET'});
  ...
  */
  notify: function(statusOrEvent, target, action) {
    ...
    //@if (debug)
    if (statusOrEvent === 'loadend' && Request.WARN_ON_LOADEND) {
      SC.warn("Developer Warning: You have called Request#notify for the 'loadend' event. Note that on certain platforms, like older iPads, loadend is not supported and this notification will fail silently. You can protect against this by checking platform.get('supportsXHR2LoadEndEvent'), and attaching listeners for load, error and abort instead. (This is not done automatically because your code may need to handle event type and fire order considerations.) To suppress this warning, set Request.WARN_ON_LOADEND to false.");
    }
    //@endif
    ...
    if (SC.typeOf(statusOrEvent) !== SC.T_NUMBER && SC.typeOf(statusOrEvent) !== SC.T_STRING) {
      ...
      args = new Array(arguments.length - 2); //  A(arguments).slice(2)
      ...
      if (arguments.length > 3) {
        // Fast arguments access.
        // Accessing `arguments.length` is just a Number and doesn't materialize the `arguments` object, which is costly.
        args = new Array(arguments.length - 3); //  A(arguments).slice(3)
...
/**
  ...
  @since SproutCore 1.0
*/
Request.manager = SC.Object.create(
/** @scope Request.manager */{
