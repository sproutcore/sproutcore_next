...
import { WebSocketDelegate } from '../mixins/websocket_delegate.js';
...
/**
  ...
  Implements SproutCore run loop aware event handling for WebSocket. Using WebSocket ensures
...
*/
export const WebSocket = SC.Object.extend(SC.DelegateSupport, WebSocketDelegate,
  /** @scope WebSocket.prototype */ {
  ...
  */
  ...
  /**
    ...
    @see WebSocketDelegate
    @type WebSocketDelegate
    @default null
  */
  delegate: null,
  ...
  */
  connect: function() {
    ...
    if (!platform.supportsWebSocket || this.socket) return this;
  ...
  */
  ...
  /**
    ...
    Example:
        ...
        var websocket = WebSocket.create({ server: 'ws://server' }).connect();
  ...
  */
  notify: function(event, target, action) {
    ...
    if (SC.typeOf(event) !== SC.T_STRING) {
      ...
      args = new Array(Math.max(0, arguments.length - 2)); //  A(arguments).slice(2)
      ...
      // Fast arguments access.
      // Accessing `arguments.length` is just a Number and doesn't materialize the `arguments` object, which is costly.
      args = new Array(Math.max(0, arguments.length - 3)); //  A(arguments).slice(3)
  ...
  /**
    ...
    If `isJSON` is true (the default for WebSocket), the message will be stringified JSON.
...
// Class Methods
WebSocket.mixin( /** @scope WebSocket */ {
