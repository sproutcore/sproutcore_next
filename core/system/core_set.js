...
*/
export const CoreSet = mixin({}, Enumerable, Freezable, Copyable,
  {
    ...
    /**
      ...
      @param {SCObject[]| CoreSet} [items] items to add
      @return {CoreSet}
    */
    create: function (items) {
      var ret, idx, pool = CoreSet._pool,
    ...
    */
    isEqual: function (obj) {
      // fail fast
      if (!obj || !obj.isCoreSet || (obj.get('length') !== this.get('length'))) {
        return false;
      }
    ...
    */
    destroy: function () {
      ...
      if (!this.isObservable) CoreSet._pool.push(this.clear());
...
/** @private */
CoreSet.clone = CoreSet.copy;

/** @private */
CoreSet.push = CoreSet.unshift = CoreSet.add;

/** @private */
CoreSet.shift = CoreSet.pop;

// add generic add/remove enumerable support

/** @private */
CoreSet.addObject = CoreSet.add;

/** @private */
CoreSet.removeObject = CoreSet.remove;
