...
import { run, RunLoop } from "./runloop.js";
...
*/
export const TaskQueue = Task.extend({
  ...
  */
  _idleEntry: function() {
    ...
    var last = RunLoop.lastRunLoopEnd;
    ...
    if (Date.now() - last > this.get('minimumIdleDuration')) {
      run(this.run, this);
      RunLoop.lastRunLoopEnd = last; // we were never here
    }
...
export const backgroundTaskQueue = TaskQueue.create({
  runWhenIdle: true
});
