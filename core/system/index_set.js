...
/**
  ...
  ---
        ...
        var set = IndexSet.create(ranges);
        set.contains(index);
        set.add(index, length);
        set.remove(index, length);

        // uses a backing Array object to return each index
...
*/
export const IndexSet = mixin({},
  Observable, Enumerable, Freezable, Copyable,
  {
    ...
    */
    ...
    _sc_validateIndexSetArguments: function (_start, _length) {
      var start = _start,
        length = _length;
      // Validate arguments. Potential bad usages include trying to call IndexSet.create() with SproutCore's standard property-hash
      // idiom, and passing in the results of bad math (NaN in particular causes problems).
      var startIsValid = false,
        lengthIsValid = false;
      // Unpack hash.
      if (typeOf(start) === T_HASH && !start.isIndexSet && none(length)) {
        length = start.length;
        start = start.start;
      }
      // Validate start.
      if (none(start)) startIsValid = true;
      else if (typeOf(start) === T_NUMBER && !isNaN(start)) startIsValid = true;
      else if (start.isIndexSet && none(length)) startIsValid = true;
      // Validate length.
      if (none(start) && !none(length)) lengthIsValid = false; // stay invalid.
      else if (none(length)) lengthIsValid = true;
      else if (typeOf(length) === T_NUMBER && !isNaN(length)) lengthIsValid = true;

      // Check validity and throw if needed.
      if (!startIsValid || !lengthIsValid) {
        var argsString = A(arguments).join(', ');
        throw new Error("IndexSet created with invalid parameters (%@). You must call IndexSet with zero, one or two valid numeric arguments.".fmt(argsString));
      }
    },
    //@endif
    ...
    /**
      ...
      @param {Number|IndexSet} start The start of the range or an index set.
    ...
    */
    rangeStartForIndex: function (index) {
      ...
      accel = index - (index % IndexSet.HINT_SIZE);
      ret = content[accel];
    ...
    */
    ...
    /**
      ...
      @param {Number|IndexSet} start index or range
    ...
    */
    ...
    /**
      ...
      @param {Number|IndexSet} start index, range, or IndexSet
      @param {Number} length optional range length
      @returns {Boolean}
    */
    ...
    /**
      ...
      @param {Number} length optional range length
      @returns {IndexSet} new index set
    */
    without: function (start, length) {
      if (start === this) return IndexSet.create(); // just need empty set
    ...
    /**
      ...
      @param {Number|IndexSet} start index, Range, or another IndexSet
      @param {Number} length optional length of range.
      @returns {IndexSet} receiver
    */
    ...
    /**
      ...
      @param {Number|IndexSet} start Start index, Range, or another IndexSet
    ...
    */
    ...
    /**
      ...
      @param {Object[]} objects The list of ranges you want to add
    */
    ...
    */
    ...
    /**
      ...
      @param {Number|IndexSet} start index, range object or IndexSet
      @param {Number} length optional range length
    ...
    forEach: function (callback, target) {
      ...
      while (next !== 0) {
        if (isFinite(next)) {
          ...
          throw new Error("Developer Error: You may not iterate an infinite range in IndexSet.");
        }
      }
      return this;
    },
