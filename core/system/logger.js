...
/**
  If {@link Logger.format} is true, this delimiter will be put between arguments.

  @type String
*/
setSetting('LOGGER_LOG_DELIMITER', ", ");

/**
  If {@link Logger.error} falls back onto {@link Logger.log}, this will be
  prepended to the output.

  @type String
*/
setSetting('LOGGER_LOG_ERROR', "ERROR: ");

/**
  If {@link Logger.info} falls back onto {@link Logger.log}, this will be
  prepended to the output.

  @type String
*/
setSetting('LOGGER_LOG_INFO', "INFO:  ");

/**
  If {@link Logger.warn} falls back onto {@link Logger.log}, this will be
  prepended to the output.

  @type String
*/
setSetting('LOGGER_LOG_WARN', "WARN:  ");

/**
  If {@link Logger.debug} falls back onto {@link Logger.log}, this will be
  prepended to the output.

  @type String
*/
setSetting('LOGGER_LOG_DEBUG', "DEBUG: ");

/**
  If {@link Logger.group} falls back onto {@link Logger.log}, this will
  be prepended to the output.

  @type String
*/
setSetting('LOGGER_LOG_GROUP_HEADER', "** %@"); // The variable is the group title
...
/**
  ...
  @type String
*/
setSetting('LOGGER_LOG_GROUP_INDENTATION', "    ");
...
/**
  ...
  @type String
*/
setSetting('LOGGER_RECORDED_LOG_TIMESTAMP_PREFIX', "%@:  ");
...
const LOGGER_LEVEL_NONE = 'none';

setSetting('LOGGER_LEVEL_DEBUG', LOGGER_LEVEL_DEBUG);
setSetting('LOGGER_LEVEL_INFO', LOGGER_LEVEL_INFO);
setSetting('LOGGER_LEVEL_WARN', LOGGER_LEVEL_WARN);
setSetting('LOGGER_LEVEL_ERROR', LOGGER_LEVEL_ERROR);
setSetting('LOGGER_LEVEL_NONE', LOGGER_LEVEL_NONE);
...
/** @instance
  ...
  stringification — using the {@link Logger.debugWithoutFmt} variants — and
  ...
  /** @namespace Logger.prototype */
    ...
    */
    ...
    /**
      ...
      If you do not specify this value, it will default to LOGGER_LEVEL_DEBUG
      when running in development mode and LOGGER_LEVEL_INFO when running in
      production mode.
    ...
    */
    ...
    /**
      ...
      If you do not specify this value, it will default to LOGGER_LEVEL_NONE.

      @type String
    */
    logRecordingLevel: LOGGER_LEVEL_NONE,
    ...
    {@link Logger.outputRecordedLogMessages} and
    {@link Logger.stringifyRecordedLogMessages} methods.
    ...
    full Object instance.  Furthermore, to minimize memory usage, niceties
...
<pre>
    Log entry:
    {
      type:               {String}     (LOGGER_LEVEL_DEBUG, etc.)
    ...
    {
      type:         {Constant}     LOGGER_LEVEL_DEBUG, etc.
  ...
  */
    ...
    */
    ...
    /**
      ...
      backwards compatibility with previous versions of Logger.  In newer
      code, you should instead set the appropriate output/recording log levels.

      If this property is set to true, it will set 'logOutputLevel' to
      LOGGER_LEVEL_DEBUG.  Otherwise, it will have no effect.
    ...
    */
    exists: function () {
      return !none(this.get('reporter'));
    }.property('reporter').cacheable(),


    /**
      If console.log does not exist, Logger will use window.alert instead
      when {@link Logger.log} is invoked.
    ...
    */
    ...
    /**
      ...
      should) use it like:
          ...
          Logger.debug("%@:  My debug message", this);       // good

      …and not:

          Logger.debug("%@:  My debug message".fmt(this));        // bad
    ...
    */
    ...
    /**
      ...
      readability until a matching {@link Logger.debugGroupEnd} is invoked,
      and you can create as many levels as you want.

      Assuming you are using 'debug' messages elsewhere, it is preferable to
      group them using this method over simply {@link Logger.group} — the log
      ...
      Thus, you can (and should) use it like:
            ...
            Logger.debugGroup("%@:  My debug group", this);       // good

      …and not:

            Logger.debugGroup("%@:  My debug group".fmt(this));   // bad
      ...
      @param {...any} optionalFormatArgs       (optional)  Other arguments to pass to String.fmt() when using a format string as the title
    */
    debugGroup: function (message, optionalFormatArgs) {
      // Implementation note:  To avoid having to put the debugGroup()
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroup(LOGGER_LEVEL_DEBUG, message, arguments);
    },


    /**
      Ends a group initiated with {@link Logger.debugGroup}, provided the
      respective output/recording log levels are set appropriately.

      @see Logger.debugGroup
    */
    debugGroupEnd: function () {
      // Implementation note:  To avoid having to put the debugGroupEnd()
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroupEnd(LOGGER_LEVEL_DEBUG);
    },
    ...
    /**
      ...
      should) use it like:
            ...
            Logger.info("%@:  My info message", this);       // good

      …and not:

            Logger.info("%@:  My info message".fmt(this));   // bad
    ...
    */
    info: function (message, optionalFormatArgs) {
      ...
      // variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleMessage(LOGGER_LEVEL_INFO, true, message, arguments);
    },
    ...
    */
    ...
    /**
      ...
      Every message after this call (at any log level) will be indented for
      readability until a matching {@link Logger.infoGroupEnd} is invoked,
      and you can create as many levels as you want.
      ...
      Thus, you can (and should) use it like:
            ...
            Logger.infoGroup("%@:  My info group", this);       // good

      …and not:

            Logger.infoGroup("%@:  My info group".fmt(this));   // bad
    ...
    */
    infoGroup: function (message, optionalFormatArgs) {
      ...
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroup(LOGGER_LEVEL_INFO, message, arguments);
    },


    /**
      Ends a group initiated with {@link Logger.infoGroup}, provided the
      respective output/recording log levels are set appropriately.

      @see Logger.infoGroup
    */
    infoGroupEnd: function () {
      // Implementation note:  To avoid having to put the infoGroupEnd()
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroupEnd(LOGGER_LEVEL_INFO);
    },
    ...
    /**
      ...
      should) use it like:
            ...
            Logger.warn("%@:  My warning message", this);       // good

      …and not:

            Logger.warn("%@:  My warning message".fmt(this));   // bad
    ...
    */
    warn: function (message, optionalFormatArgs) {
      ...
      // variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleMessage(LOGGER_LEVEL_WARN, true, message, arguments);
    ...
    */
    ...
    /**
      ...
      Every message after this call (at any log level) will be indented for
      readability until a matching {@link Logger.warnGroupEnd} is invoked,
      and you can create as many levels as you want.
      ...
      Thus, you can (and should) use it like:
            ...
            Logger.warnGroup("%@:  My warn group", this);       // good

      …and not:

            Logger.warnGroup("%@:  My warn group".fmt(this));   // bad
    ...
    */
    warnGroup: function (message, optionalFormatArgs) {
      ...
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroup(LOGGER_LEVEL_WARN, message, arguments);
    },


    /**
      Ends a group initiated with {@link Logger.warnGroup}, provided the
      respective output/recording log levels are set appropriately.

      @see Logger.warnGroup
    */
    warnGroupEnd: function () {
      // Implementation note:  To avoid having to put the warnGroupEnd()
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroupEnd(LOGGER_LEVEL_WARN);
    },
    ...
    /**
      ...
      should) use it like:
            ...
            Logger.error("%@:  My error message", this);       // good

      …and not:

            Logger.warn("%@:  My error message".fmt(this));    // bad
    ...
    */
    error: function (message, optionalFormatArgs) {
      ...
      // variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleMessage(LOGGER_LEVEL_ERROR, true, message, arguments);
    },
    ...
    */
    ...
    /**
      ...
      Every message after this call (at any log level) will be indented for
      readability until a matching {@link Logger.errorGroupEnd} is invoked,
      and you can create as many levels as you want.
      ...
      Thus, you can (and should) use it like:
            ...
            Logger.errorGroup("%@:  My error group", this);       // good

      …and not:

            Logger.errorGroup("%@:  My error group".fmt(this));   // bad
    ...
    */
    errorGroup: function (message, optionalFormatArgs) {
      ...
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroup(LOGGER_LEVEL_ERROR, message, arguments);
    },


    /**
      Ends a group initiated with {@link Logger.errorGroup}, provided the
      respective output/recording log levels are set appropriately.

      @see Logger.errorGroup
    */
    errorGroupEnd: function () {
      // Implementation note:  To avoid having to put the errorGroupEnd()
      // shorthand variant inside a function wrapper, we'll avoid 'this'.
      Logger._handleGroupEnd(LOGGER_LEVEL_ERROR);
    },
    ...
    /**
      ...
      included as a prefix if you specify 'includeTimestamps' as true, although
    ...
    */
    stringifyRecordedLogMessages: function () {
      var ret = "",
        ...
        groupHeader = getSetting('LOGGER_LOG_GROUP_HEADER'),
    ...
    */
    log: function () {
      ...
      if (message && typeOf(message) === T_STRING) {
    ...
    /**
      Every log after this call until {@link Logger.groupEnd} is called
    ...
    */
    ...
    /**
      Ends a group declared with {@link Logger.group}.
    ...
    */
    ...
    init: function init () {
      ...
      if (!this.get('logOutputLevel')) {
        if (getSetting('buildMode') === "debug") {
          this.set('logOutputLevel', LOGGER_LEVEL_DEBUG);
        } else {
          this.set('logOutputLevel', LOGGER_LEVEL_INFO);
        }
      }
    ...
    /** @private
      ...
      our log output level to LOGGER_LEVEL_DEBUG if 'debugEnabled' is set to
      true.
    */
    ...
    /** @private
      ...
      @param {String}  [originalArguments]    All arguments passed into debug(), etc. (which includes 'message'; for efficiency, we don’t copy it)
    */
    _handleMessage: function (type, automaticallyFormat, message, originalArguments) {
      ...
      if (automaticallyFormat && (none(message) || (typeof message !== "string"))) {
        automaticallyFormat = false;
      }
      ...
      if (shouldRecord) {
        ...
        entry = {
          type: type,
          message: message ? (recordedPrefix ? recordedPrefix + message : message) : true,
          timestamp: new Date(),
    ...
    */
    _outputGroup: function (type, timestampStr, indentation, title) {
      ...
      if (func) {
        ...
        output += getSetting('LOGGER_LOG_GROUP_HEADER').fmt(title);
        reporter.log(output);
      }
    },
    ...
    */
    ...
    /** @private
      ...
      LOGGER_LOG_DELIMITER between the elements.
    */
    _argumentsToString: function () {
      var ret = "",
        delimiter = getSetting('LOGGER_LOG_DELIMITER'),
        i, len;
    ...
    */
    _indentation: function (level) {
      ...
      var ret = "",
        indent = getSetting('LOGGER_LOG_GROUP_INDENTATION'),
        i;
    ...
    */
    ...
    /** @private
      A mapping of the log level constants (LOGGER_LEVEL_DEBUG, etc.) to
    ...
    */
    _LOG_FALLBACK_PREFIX_MAPPING: {
      debug: getSetting('LOGGER_LOG_DEBUG'),
      info: getSetting('LOGGER_LOG_INFO'),
      warn: getSetting('LOGGER_LOG_WARN'),
      error: getSetting('LOGGER_LOG_ERROR')
    }

  });


// Add convenient shorthands methods to
