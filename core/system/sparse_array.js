...
export const SparseArray = SCObject.extend(Observable, Enumerable, SCArray,
  DelegateSupport, /** @scope SparseArray.prototype */ {
  ...
  */
  length: function() {
    ...
    if (del && none(this._length) && del.sparseArrayDidRequestLength) {
  ...
  /**
    ...
    @returns {SparseArray} receiver
  */
  provideLength: function(length) {
    var oldLength;
    if (none(length)) this._sa_content = null ;
  ...
  */
  ...
  /**
    ...
    and will return true (or undefined if not found). This is useful in the case
    of SparseArrays, where you might falseT want to request the index to be loaded,
  ...
  */
  ...
  /**
    ...
    @param {IndexSet} indexes optional from indexes
    @returns {IndexSet} defined indexes
  */
  definedIndexes: function(indexes) {
    var ret = IndexSet.create(),
  ...
  */
  objectsDidChangeInRange: function(range) {
    ...
    if (content) {
      ...
      if (range.start === 0 && maxRange(range)>=content.length) {
...
/**
  ...
  @returns {SparseArray}
*/
SparseArray.array = function(len) {
