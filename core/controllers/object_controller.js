...
import '../ext/string.js'; // to use fmt
import { none, typeOf } from '../system/base.js';
import { T_FUNCTION } from '../system/constants.js';
...
*/
export const ObjectController = Controller.extend({
  //@if(debug)
  ...
  */
  hasContent: function() {
    return !none(this.get('observableContent'));
  ...
  */
  ...
  /**
    ...
    @returns {ObjectController} receiver
  */
  destroy: function() {
    var content = this.get('observableContent') ;
    if (content && typeOf(content.destroy) === T_FUNCTION) {
      content.destroy();
    }
