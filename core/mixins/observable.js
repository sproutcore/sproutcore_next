...
//@if(debug)
/**
  Set to true to have all observing activity logged to the console.
...
setSetting('LOG_OBSERVERS', false);
//@endif

const OBSERVES_HANDLER_ADD = 0;
const OBSERVES_HANDLER_REMOVE = 1;
// setSetting('OBSERVES_HANDLER_ADD', OBSERVES_HANDLER_ADD);
// setSetting('OBSERVES_HANDLER_REMOVE', OBSERVES_HANDLER_ADD);
...
/**
  ...
  changes:
        ...
        automaticallyNotifiesObserversFor: function automaticallyNotifiesObserversFor (key) {
          return (key === 'balance') ? false : automaticallyNotifiesObserversFor.super.apply(this, arguments);
        },
...
export const Observable = {
  ...
  */
  addProbe: function (key) {
    this.addObserver(key, logChange);
  },
  ...
  */
  removeProbe: function (key) {
    this.removeObserver(key, logChange);
  },
  ...
  */
  logProperty: function (...propertyNames) {
    for (let idx = 0, propsLen = propertyNames.length; idx < propsLen; idx++) {
      let prop = propertyNames[idx];
      console.log('%@:%@: '.fmt(guidFor(this), prop), this.get(prop));
    }
  },
  ...
  */
  endPropertyChanges: function () {
    ...
    var level = this._kvo_changeLevel,
      changes = this._kvo_changes;
    if ((level <= 0) && changes && (changes.length > 0) && !ObserverQueue.isObservingSuspended) {
  ...
  */
  propertyDidChange: function (key, value, _keepCache) {
    ...
    //@if(debug)
    var log = getSetting('LOG_OBSERVERS') && (this.LOG_OBSERVING !== false);
    //@endif
    ...
    var suspended = ObserverQueue.isObservingSuspended;
    if ((level > 0) || suspended) {
      var changes = this._kvo_changes;
      if (!changes) changes = this._kvo_changes = CoreSet.create();
      changes.add(key);

      if (suspended) {
        //@if(debug)
        if (log) console.log("%@%@: will not notify observers because observing is suspended".fmt(getSetting('KVO_SPACES'), this));
        //@endif
        ObserverQueue.objectHasPendingChanges(this);
      }
  ...
  */
  registerDependentKey: function (key, dependentKeys) {
    ...
    while (--idx >= lim) {
      ...
      if (dep.indexOf('.') >= 0) {
        PropertyChain.createChain(dep, this, key).activate();
      } else {
  ...
  /** @private
    ...
    @param {PropertyChain} chain the chain to notify
  */
  ...
  */
  _chainsFor: function (property) {
    ...
    var chains = this._kvo_property_chains[property] || CoreSet.create();
  ...
  */
  _kvo_computeCachedDependentsFor: function (key) {
    ...
    queue = cached[key] = [];
    seen = _TMP_SEEN_SET = (_TMP_SEEN_SET || CoreSet.create());
    seen.add(key);
  ...
  */
  addObserver: function (key, target, method, context) {
    ...
    if (key.indexOf('.') >= 0) {
      ...
      // needed.
      chain = ChainObserver.createChain(this, key, target, method, context);
      ...
      this._kvo_for(kvoKey, ObserverSet).add(target, method, context);
      this._kvo_for('_kvo_observed_keys', CoreSet).add(key);
    }
  ...
  */
  hasObserverFor: function (key, target, method) {
    ObserverQueue.flush(this); // hookup as many observers as possible.
    ...
    if (target === undefined) {
      ...
      if (isChain) {
        ...
        len = chains.length;
        for (i = 0; i < len; i++) {
          if (chains[i].masterTarget === target && chains[i].masterMethod === method) return true;
        }
        return false;
        ...
        len = locals.length;
        for (i = 0; i < len; i++) {
          if (this[locals[i]] === method) return true;
        }
        return false;

        // Check remotes.
      } else {
        if (!observers || !observers.members) return false;
        ...
        for (i = 0; i < len; i++) {
          ...
          if (member[0] === target && member[1] === method) return true;
        }
        return false;
      }
      // TODO: Remote chains.
  ...
  /**
    ...
    from your own initialization method if you are using Observable in
  ...
  */
  addObservesHandler: function (observer, path) {
    this._configureObservesHandler(OBSERVES_HANDLER_ADD, observer, path);
    return this;
  },
  ...
  */
  removeObservesHandler: function (observer, path) {
    this._configureObservesHandler(OBSERVES_HANDLER_REMOVE, observer, path);
    return this;
  },
  ...
  */
  _configureObservesHandler: function (action, observer, path) {
    ...
    switch (action) {
      case OBSERVES_HANDLER_ADD:
        action = "addObserver";
        break;
      case OBSERVES_HANDLER_REMOVE:
    ...
    if (dotIndex < 0) {
      ...
      ObserverQueue[action](path, this, observer, root);
    }
  },
  ...
  */
  _notifyPropertyObservers: function (key) {
    ...
    var log = getSetting('LOG_OBSERVERS') && this.LOG_OBSERVING !== false;
    if (log) {
      let KVO_SPACES = getSetting('KVO_SPACES') || '';
      spaces = KVO_SPACES + '  ';
      setSetting('KVO_SPACES', spaces);
    ...
    while (((changes = this._kvo_changes) && (changes.length > 0)) || key) {
      ...
      if (!changes) changes = CoreSet.create();
      ...
      if (key === '*') {
        changes.add('*');
        changes.addEach(this._kvo_for('_kvo_observed_keys', CoreSet));
    ...
    //@if(debug)
    if (log) setSetting('KVO_SPACES', spaces.slice(0, -2));
    //@endif
  ...
  /**
    ...
    @param {Object|String} target target or property path to bind from
  ...
  */
  bind: function (toKey, target, method) {
    ...
    if (pathType === "string" || (pathType === "object" && (target instanceof Array))) {
      binding = this[toKey + 'BindingDefault'] || Binding;
  ...
  /**
    ...
    @param {...String} propertyNames one or more property names
  */
  didChangeFor: function (context, ...propertyNames) {
    var valueCache, revisionCache, seenValues, seenRevisions, ret,
      currentRevision, idx, key, value;
    context = hashFor(context); // get a hash key we can use in caches.
    ...
    while (--idx >= 1) { // NB: loop only to 1 to ignore context arg.
      ...
      if (seenRevisions[key] != currentRevision) {
        // true, check the value with the last seen value
        value = this.get(key);
  ...
  */
  getPath: function (path) {
    var tuple = tupleForPropertyPath(path, this);
...
/**
  ...
  object implements Observable.

  @param  {Object}  object  the object to query
  @param  {String}  key the property to retrieve
*/
export const get = function (object, key) {
  ...
  /**
    ...
    the object implements Observable.
