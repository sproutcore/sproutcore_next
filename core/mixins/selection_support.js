...
/**
  ...
  This mixin assumes the arrangedObjects property will return an Array of
  content you want the selection to reflect.

  Add this mixin to any controller you want to manage selection.  It is
  already applied to ArrayController.

  @since SproutCore 1.0
*/
export const SelectionSupport = {
  ...
  */
  selection: function(key, value) {
    ...
    if ((len > 1) && !this.get('allowsMultipleSelection')) {
      ...
      if (oldlen > 1) {
        value = SelectionSet.create().addObject(old.get('firstObject')).freeze();
        len = 1;
      } else {
        value = old;
        len = oldlen;
    ...
    if ((len === 0) && !this.get('allowsEmptySelection') && arrangedObjects && arrangedObjects.get('length') !== 0) {
      if (oldlen === 0) {
        ...
        if (value) { value = SelectionSet.create().addObject(value).freeze(); }
        else { value = SelectionSet.EMPTY; }
    ...
    if (len === 0) { value = SelectionSet.EMPTY; }
  ...
  */
  ...
  /**
    ...
    @param {Enumerable} objects objects to select
  ...
  */
  selectObjects: function(objects, extend) {
    ...
    if (!objects || objects.get('length') === 0) {
      if (!extend) { this.set('selection', SelectionSet.EMPTY); }
      return this;
    }

    var sel = this.get('selection');
    if (extend && sel) { sel = sel.copy(); }
    else { sel = SelectionSet.create(); }
  ...
  */
  ...
  /**
    ...
    @param {Object} object single object to select
    @returns {Object} receiver
  */
