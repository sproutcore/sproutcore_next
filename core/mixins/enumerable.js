...
import { SCEnumerator } from '../system/enumerator.js';
import { T_STRING, T_FUNCTION } from '../system/constants.js';
import { compare, isEqual, none, typeOf, mixin } from '../system/base.js';
...
export const Enumerable = {
  ...
  */
  firstObject: function () {
    ...
    var context = SCEnumerator._popContext(),
      ret;
    ret = this.nextObject(0, null, context);
    context = SCEnumerator._pushContext(context);
    return ret;
  }.property().cacheable(),
  ...
  */
  ...
  /**
    Returns a new enumerator for this object.  See SCEnumerator for
    documentation on how to use this object.  Enumeration is an alternative
    to using one of the other iterators described here.

    @returns {SCEnumerator} an enumerator for the receiver
  */
  enumerator: function () {
    return SCEnumerator.create(this);
  },
  ...
  */
  sortProperty: function (key) {
    var keys = (typeof key === T_STRING) ? arguments : key,
      len = keys.length,
      src;
  ...
  */
  filterProperty: function (key, value) {
    var len = this.get ? this.get('length') : this.length,
      ret = [],
      last = null,
      context = SCEnumerator._popContext(),
      idx, item, cur;
  ...
  */
  ...
  /**
    ...
    @returns {Array} return values from calling invoke.
  */
  invokeWhile: function (targetValue, methodName, ...args) {
    var len = this.get ? this.get('length') : this.length;
...
export const _buildReducerFor = function (reducerKey, reducerProperty) {
  return function (key, value) {
    var reducer = this[reducerKey];
    if (typeOf(reducer) !== T_FUNCTION) {
      ...
      var ret = Enumerable.reduce.call(this, reducer, null, reducerProperty);
      return ret;
    }
  }.property('[]');
};
...
export const Reducers = {
  ...
  */
  enumerableContentDidChange: function (start, length, deltas) {
    ...
    if (!none(start)) {
      ...
      if (!none(length) && start + length >= this.get('length') - 1) this.notifyPropertyChange('lastObject');
    }
  ...
  */
  reducedProperty: function (key, value, generateProperty) {
    ...
    if (!key || typeof key !== T_STRING || key.charAt(0) !== '@') return undefined; // not a reduced property
    ...
    if (typeOf(reducer) !== T_FUNCTION) return undefined;

    // if we can't generate the property, just run reduce
    if (generateProperty === false) {
      return Enumerable.reduce.call(this, reducer, null, reducerProperty);
    }

    // ok, found the reducer.  Let's build the computed property and install
    var func = _buildReducerFor(reducerKey, reducerProperty);
  ...
  */
  reduceMaxObject: function (previousItem, item, index, e, reducerProperty) {
    ...
    var previousValue = previousItem,
      itemValue = item;
    if (reducerProperty) {
      if (item) {
