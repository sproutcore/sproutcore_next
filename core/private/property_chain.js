...
export const PropertyChain = SCObject.extend({
    /**
...
PropertyChain.createChain = function (path, target, toInvalidate) {
  var parts = path.split('.');
  var len = parts.length,
    i = 1;

  var root = PropertyChain.create({
    property: parts[0],
    target: target,
  ...
  while (--len >= 1) {
    tail = tail.next = PropertyChain.create({
      property: parts[i],
      target: target,
      toInvalidate: toInvalidate,
