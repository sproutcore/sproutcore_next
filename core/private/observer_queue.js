...
export const ObserverQueue = {
  ...
  */
  removeObserver: function (propertyPath, target, method, pathRoot) {
    ...
    // Add some developer support indicating that the observer was not removed.
      ...
      console.warn("Developer Warning: Attempted to remove observer for propertyPath %@ with root %@ and failed.  This may be because an object that the path refers to is no longer available.".fmt(propertyPath, pathRoot));
    }
    //@endif
  ...
  */
  addPendingRangeObserver: function (observer) {
    ...
    if (!ro) ro = this.rangeObservers = CoreSet.create();
    ro.add(observer);
    return this;
  },
