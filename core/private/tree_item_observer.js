...
*/
export const TreeItemObserver = SCObject.extend(SCArray, CollectionContent, {
  ...
  */
  ...
  /**
    Disclosure state of this item.  Must be BRANCH_OPEN or BRANCH_CLOSED
  ...
  branchIndexes: function () {
    ...
    // otherwise, loop over children and determine disclosure state for each
      ...
      for (idx = 0; idx < len; idx++) {
        ...
        if (this._computeDisclosureState(item, pitem, idx) !== LEAF_NODE) {
          ret.add(idx);
        }
      }
  ...
  */
  ...
  /**
    Implements Array.replace() primitive.  For this method to succeed, the
    ...
    DROP_BEFORE, which is the expected behavior.

    Even if the operation is DROP_AFTER, you should still pass the actual
    ...
    to insert AFTER the last index of an 3-item array, you would still call:
        ...
        observer.replace(3, 0, [object1 .. objectN], DROP_AFTER)
    ...
    @param {Number} operation either DROP_BEFORE or DROP_AFTER
    @returns {TreeItemObserver} receiver
  */
  replace: function (start, amt, objects, operation) {
    ...
    if (operation === undefined) operation = DROP_BEFORE;
    ...
    if (indexes) {
      indexes.forEach(function (i) {
        ...
        if ((i + len === cur) && operation === DROP_AFTER) {
          cur = cur - i;
        } else if (i + len > cur) {
  ...
  */
  observerContentDidChange: function (start, amt, delta) {
    ...
    if (parent) {
      set = IndexSet.create(this.get('index'));
  ...
  */
  contentIndexDisclosureState: function (view, content, index) {
    ...
    if (cur >= 0) ret = LEAF_NODE; // otherwise its a leaf node
    cache[index] = ret; // save in cache
    return ret;
  },
  ...
  */
  branchObserverAt: function (index) {
    ...
    if (!indexes) {
      indexes = this._branchObserverIndexes = IndexSet.create();
    }
    ...
    byIndex[index] = ret = TreeItemObserver.create({
      item:     item,
  ...
  */
  destroy: function destroy () {
    ...
    this._notifyParent = false; // parent doesn't care anymore
  ...
  _itemDidChange: function () {
    ...
    if (!item) { throw new Error("Developer Error: TreeItemObserver: Item cannot be null and must be set on create."); }

    if (item.hasObserverFor(treeItemIsExpandedKey)) {
      warn("Developer Warning: TreeItemObserver: Item '%@' appears to already be assigned to a tree item observer. This will cause strange behavior working with the item.".fmt(item));
    }
    //@endif
  ...
  */
  _childrenDidChange: function () {
    var state = this.get('disclosureState'),
      cur   = state === BRANCH_OPEN ? this.get('children') : null,
      last  = this._children,
      ro    = this._childrenRangeObserver;
  ...
  */
  _computeDisclosureState: function (item, pitem, index) {
    ...
    if (!item || !this._computeChildren(item)) return LEAF_NODE;
    ...
    // otherwise get treeItemDisclosureStateKey from delegate
      ...
      return item.get(key) ? BRANCH_OPEN : BRANCH_CLOSED;
    }
  },
