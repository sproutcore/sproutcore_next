...
// WARNING, depends on global QUnit

const module = QUnit.module;
const assert = QUnit.assert;
const ok = assert.ok.bind(assert);
const equals = assert.equal.bind(assert);
...
/**
  ...
  the Array interface.  To generate, call the ArrayTests array with a 
...
export const TestSuite = /** @scope TestSuite.prototype */ {

  /**
    Call this method to define a new test suite.  Pass one or more hashes of
    properties you want added to the new suite.  
    
    @param {Object} attrs one or more attribute hashes
    @returns {TestSuite} subclass of suite.
  */
  ...
  /**
    ...
    @param {Object} attrs one or more attribute hashes
    @returns {TestSuite} suite instance
  */
  ...
  */
  module: function(desc) {
    var T = this ;
    module(T.desc(desc), {
      beforeEach: function() { T.setup(); },
      afterEach: function() { T.teardown(); }
    });
  }
  
};

export const ArraySuite = TestSuite.create("Verify Array compliance: %@#%@", {
...
ArraySuite.define(function(T) {
  ...
  test("should return 0 on empty array", function(assert) {
    assert.equal(T.object.get('length'), 0, 'should have empty length');
  });
  
  test("should return array length", function(assert) {
    var obj = T.newObject(3);
    assert.equal(obj.get('length'), 3, 'should return length');
  });
