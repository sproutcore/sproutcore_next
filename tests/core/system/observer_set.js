...
test("Observers that remove themselves should fire at least once, and shouldn't cause an error", function (assert) {
  assert.expect(1);
  ...
  assert.ok(observerFired, "At least one observer should have fired.");
});
...
/**
  ...
  coreOb._kvo_observers_key._members:
    ...
    obs = [];
    GLOBAL.coreOb = SC.Object.create({ key: '1' });
    SC.run(function() {
...
test("Removing all the methods on a target should clear the internal tracking of that target", function (assert) {
  ...
  assert.equal(observerSet.members.length, 0, "The ObserverSet should have a members length of");

  observerSet.add(target, method1);
  observerSet.add(target, method2);
  assert.equal(observerSet.members.length, 2, "The ObserverSet should have a members length of");
  assert.ok(observerSet._members[targetGuid], "The ObserverSet should be tracking methods for the target: %@".fmt(targetGuid));
  assert.ok(!SC.none(observerSet._members[targetGuid][methodGuid1]), "The ObserverSet should be tracking method: %@ for the target: %@".fmt(methodGuid1, targetGuid));
  assert.ok(!SC.none(observerSet._members[targetGuid][methodGuid2]), "The ObserverSet should be tracking method: %@ for the target: %@".fmt(methodGuid2, targetGuid));

  observerSet.remove(target, method1);
  assert.equal(observerSet.members.length, 1, "The ObserverSet should have a members length of");
  assert.ok(observerSet._members[targetGuid], "The ObserverSet should be tracking methods for the target: %@".fmt(targetGuid));
  assert.ok(SC.none(observerSet._members[targetGuid][methodGuid1]), "The ObserverSet should not be tracking method: %@ for the target: %@".fmt(methodGuid1, targetGuid));
  assert.ok(!SC.none(observerSet._members[targetGuid][methodGuid2]), "The ObserverSet should be tracking method: %@ for the target: %@".fmt(methodGuid2, targetGuid));
