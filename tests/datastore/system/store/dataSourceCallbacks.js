...
module("Store#dataSourceCallbacks", {
  beforeEach: function() {
...
*/
test("Confirm that passing a new id to dataSourceDidComplete clears the cached id on the record", function (assert) {
  var MyRecordType = Record.extend({});

  // Assign the store key to a record type so we can materialize the record.
  Store.replaceRecordTypeFor(storeKey11, MyRecordType);
  Store.idsByStoreKey[storeKey11] = 'commitGUID11';

  var record = store.materializeRecord(storeKey11),
      tempId = record.get('id'),
      body;

  assert.equal(tempId, 'commitGUID11', "The id should originally be");
  ...
  assert.equal(record.get('id'), 'actualID', "The id should update now to be");
});
