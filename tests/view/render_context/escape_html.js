...
test("Escaping HTML", function (assert) {
  var input = "<p>HTML!</p><script>alert('hi');<" + "/script> & Hello, World!";
  var output = RenderContext.escapeHTML(input);
  
  assert.equal(output, '&lt;p&gt;HTML!&lt;/p&gt;&lt;script&gt;alert(\'hi\');&lt;/script&gt; &amp; Hello, World!', "Properly escapes HTML");
});

module("Render Context--Escaping , preserve HTML entities like &apos;");
test("Escaping HTML, preserve HTML entities", function (assert) {
  var input = "<p>HTML!</p><script>alert('hi');<" + "/script> &illegalese; & &amp; Hello, World!";
  var output = RenderContext.escapeHTML(input);
  
  assert.equal(output, '&lt;p&gt;HTML!&lt;/p&gt;&lt;script&gt;alert(\'hi\');&lt;/script&gt; &amp;illegalese; &amp; &amp; Hello, World!', "Properly escapes HTML");
});

test("Tests stolen from Prototype.js", function (assert) {
  ...
  for (var idx = 0; idx < tests.length; idx++) {
    ...
    assert.ok(RenderContext.escapeHTML(tests[idx++]) === tests[idx]);
  }
});

test("Should accept number argument", function (assert) {
  var number = 12345.6789,
      numStr = number.toString();
  
  assert.equal(numStr, RenderContext.escapeHTML(number), "Properly produces string when invoked with a number argument");
});
