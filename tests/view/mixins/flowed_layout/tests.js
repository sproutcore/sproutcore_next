...
import { View, LAYOUT_HORIZONTAL, LAYOUT_VERTICAL, ALIGN_LEFT, ALIGN_CENTER, ALIGN_RIGHT, ALIGN_JUSTIFY, FlowedLayout } from '../../../../view/view.js';
...
var checkSteps = [
  function wrap(view, positions, context) {
    SC.RunLoop.begin();
    view.set('canWrap', true);
    SC.RunLoop.end();

    context.performNext(positions.wrap);

    SC.RunLoop.begin();
    view.set('canWrap', false);
    SC.RunLoop.end();
  ...
  function orientation(view, positions, context) {
    ...
    SC.RunLoop.begin();
    view.set('layoutDirection', LAYOUT_VERTICAL);
    flipChildViews();
    flipLayout();
    SC.RunLoop.end();
  ...
  function final(view, positions, context) {
    var cvs = view.get('childViews');
    assert.equal(cvs.length, positions.length, "PRECONDITION: number of child views is the same.");

    for (var idx = 0; idx < positions.length; idx++) {
      var cv = cvs[idx], expect = positions[idx], equal = true;
      for (var i in expect) {
...
test("Basic flow", function (assert) {
  var view = View.create(FlowedLayout, {
    childViews: 'a b c'.w(),
...
test("Align center", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
...
test("Align right", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
...
test("Align justify", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
...
test("flowPadding", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
...
test("FlowedLayout - defaultFlowSpacing", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
...
test("startsNewRow", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
  ...
  SC.RunLoop.begin();
  view.childViews[1].set('startsNewRow', true);
  SC.RunLoop.end();
...
test("flowSpacing", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
...
test("fillHeight", function (assert) {
  var view = View.create(FlowedLayout, {
    align: ALIGN_LEFT,
  ...
  SC.RunLoop.begin();
  view.childViews[1].set('fillHeight', true);
  SC.RunLoop.end();
...
test("isSpacer", function (assert) {
  ...
  SC.RunLoop.begin();
  view.childViews[1].set('isSpacer', true);
  SC.RunLoop.end();
...
test("undefined and null can be passed for both defaultFlowSpacing and child's flowSpacing", function (assert) {
  var view = View.create(FlowedLayout, {
    childViews: 'a b c'.w(),
...
test("Observed childViews", function (assert) {
  var view;

  SC.run(function() {
    view = View.create(FlowedLayout, {
      childViews: 'a b c'.w(),
  ...
  assert.ok(view._scfl_isObserving.contains(view.a), "FlowedLayout view should observe all three childViews (a).");
  assert.ok(view._scfl_isObserving.contains(view.b), "FlowedLayout view should observe all three childViews (b).");
  assert.ok(view._scfl_isObserving.contains(view.c), "FlowedLayout view should observe all three childViews (c).");
  ...
  assert.ok(!view._scfl_isObserving.contains(view.b), "FlowedLayout view should no longer observe the layout of a removed child view (b).");
  assert.ok(view._scfl_isObserving.contains(view.a), "FlowedLayout view should still be observing all three childViews (a).");
  assert.ok(view._scfl_isObserving.contains(view.c), "FlowedLayout view should still be observing all three childViews (c).");
