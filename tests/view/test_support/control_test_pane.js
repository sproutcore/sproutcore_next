...
/** @class
  ...
  # Example
      ...
      var pane = ControlTestPane.design()
        .add('basic', CheckboxView.design({ title: "Hello World" }))
        .add('disabled', CheckboxView.design({
...
*/
export const ControlTestPane = Pane.extend(
/** @scope ControlTestPane.prototype */ {
  ...
  */
  view: function(keyName) {
    ...
    if (!idx) throw new Error("ControlTestPane does not have a view named %@".fmt(keyName));
...
/**
  ...
  @param {View} view a view class or view design
  @param {Object} attrs optional attrs to use when designing the view
  @returns {ControlTestPane} receiver
*/
ControlTestPane.add = function(label, view, attrs) {
  ...
  var labelView = LabelView.design({
    value: label + ':',
    layout: { left: 0, right: 0, top: 0, height: labelHeight },
   // TODO: textAlign: ALIGN_RIGHT,
    // TODO: fontWeight: BOLD_WEIGHT
  });

  // wrap label in parent view in order to center text vertically
  labelView = View.design().layout(layout).childView(labelView);
  this.childView(labelView);

  // now layout view itself...
  var wrapper = View.design({
    classNames: ['wrapper'],
...
*/
ControlTestPane.standardSetup = function() {
  var pane = this ;
  return {
    beforeEach: function() {
      SC.RunLoop.begin();
...
/**
  ...
  @returns {View} view instance
*/
ControlTestPane.view = function(viewKey) {
...
*/
ControlTestPane.becomeKeyPane = function () {
  var pane = this._pane || this._showPane;
  if (pane) {
    SC.run(function () {
      pane.becomeKeyPane();
