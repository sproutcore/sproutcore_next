...
export const ManyArray = SC.Object.extend(SC.Enumerable, SC.Array,
  /** @scope ManyArray.prototype */ {
  ...
  toString: function toStr () {
    ...
    return "%@({\n  ids: [%@],\n  length: %@,\n  … })".fmt(sup, readOnlyStoreIds, length);
  },
  ...
  */
  objectAt: function (idx) {
    ...
    if (store.readStatus(storeKey) === Record.EMPTY) {
  ...
  */
  replace: function (idx, amt, recs) {
    ...
    if (inverse && amt > 0) {
      ...
      toRemove = ManyArray._toRemove;
      if (toRemove) ManyArray._toRemove = null; // reuse if possible
      else toRemove = [];
    ...
    if (inverse) {
      ...
      if (toRemove) {
        ...
        if (!ManyArray._toRemove) ManyArray._toRemove = toRemove;
      }
  ...
  /**
    ...
    @param {Record} inverseRecord the record that was removed
    @returns {ManyArray} receiver
  */
  ...
  /**
    ...
    @param {Record} inverseRecord the record this array is a part of
    @returns {ManyArray} receiver
  */
