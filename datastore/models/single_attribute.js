...
/** @class
  ...
  There are many ways you can configure a `SingleAttribute`:
      ...
      group: Record.toOne('MyApp.Group', {
...
*/
export const SingleAttribute = RecordAttribute.extend(
  /** @scope SingleAttribute.prototype */ {
  ...
  */
  call: function(record, key, newRec) {
    ...
    if (newRec !== undefined && this.get('isEditable')) {
      ...
      //@if(debug)
      if (newRec && !SC.kindOf(newRec, Record)) {
        throw new Error("Developer Error: %@ is not an instance of Record.".fmt(newRec));
      }
  ...
  /**
    ...
    @param {Record} record the record owning this attribute
