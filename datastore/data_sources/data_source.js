...
/** @class
  ...
  To connect to a data backend on a server, subclass `DataSource`
  and implement the necessary data source methods to communicate with the
  particular backend.

  ## Create a Data Source

  To implement the data source, subclass `DataSource` in a file located
  ...
  In core.js:
      ...
      // to...
      store: Store.create().from('MyApp.DataSource')
  ...
  All of the methods you implement must return one of three values:
   ...
   - `MIXED_STATE` &mdash; some, but not all of the records were handled.
   ...
   * `readStatus(storeKey)` &mdash; returns the current record status
     associated with the store key. May be `Record.EMPTY`.
   * `Store.recordTypeFor(storeKey)` &mdash; returns the record type for
     the associated store key.
   * `recordType.idFor(storeKey)` &mdash; returns the record ID for
     the associated store key. You must call this method on `Record`
     subclass itself, not on an instance of `Record`.
...
*/
export const DataSource = SC.Object.extend( /** @scope DataSource.prototype */ {
  ...
  /**
    ...
    you invoke Store#find() with a query or RecordArray#refresh().  You
    ...
    you should always call Store#dataSourceDidFetchQuery() to put the query
    ...
    Store#dataSourceDidFetchQuery() with the array of storeKeys that
    represent the latest results from the server.

    If you want to support incremental loading from the server for remote
    queries, you can do so by passing a SparseArray instance instead of
    ...
    you can call Store#dataSourceDidErrorQuery() instead.  This will put
    the query results into an error state.

    If you had to cancel fetching a query before the results were returned,
    you can instead call Store#dataSourceDidCancelQuery().  This will set
    ...
    @param {Store} store the requesting store
    @param {Query} query query describing the request
  ...
  */
  ...
  /**
    ...
    `MIXED_STATE` if you can handle some of them.
    ...
    @param {Object} params to be passed down to data source. originated
  ...
  */
  ...
  /**
    ...
    `MIXED_STATE` if you can retrieve some of the.
  ...
  _handleEach: function(store, storeKeys, action, ids, params) {
    ...
    for(idx=0;idx<len;idx++) {
      ...
      if (ret === undefined) {
        ...
        ret = (cur === true) ? true : MIXED_STATE ;
      } else if (ret === false) {
        ret = (cur === false) ? false : MIXED_STATE ;
      }
    }
