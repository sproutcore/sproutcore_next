...
/**
  ...
  `CollectionRowDelegate`s are consulted by collection views, such as `ListView`
  that lay out items in vertical or horizontal rows, in order to determine the
  height or width of each row.
*/
export const CollectionRowDelegate = {
  ...
  */
  ...
  rowSize: function (key, value) {
    ...
    if (!SC.none(itemHeight)) {
      //@if(debug)
      SC.warn('Developer Warning: The itemHeight property of CollectionRowDelegate has been renamed to rowSize.');
      //@endif
  ...
  */
  ...
  customRowSizeIndexes: function (key, value) {
    ...
    if (!SC.none(customRowHeightIndexes)) {
      //@if(debug)
      SC.warn('Developer Warning: The customRowHeightIndexes property of CollectionRowDelegate has been renamed to customRowSizeIndexes.');
      //@endif
  ...
  /** @deprecated Version 1.11. Please use the `contentIndexRowSize()` function instead.
    ...
    @param {CollectionView} view the calling view
