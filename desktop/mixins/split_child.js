...
import { LAYOUT_HORIZONTAL, propertyFromRenderDelegate } from '../../view/view.js';

/** @private
  SplitChild and SplitThumb both share this code, but we don't want it
  included twice if the view mixes in both SplitChild and SplitThumb.
*/
export const NeedsSplitParent = {
...
export const SplitChild =
  /** @scope SplitChild.prototype */{
  /**
   * Set to true if your Split Child is a divider view.
   *
   * @type Boolean
  */
  isSplitDivider: false,

  /**
   * The minimum size for the SplitView child. This view will
  ...
  */
  ...
  /**
   * The maximum size for the SplitView child. This view will
  ...
  */
  ...
  /**
   * The current size of the SplitView child. Use this to set the default
   * size.
   *
   * If you specify a size, autoResizeStyle will default to RESIZE_MANUAL;
   * if you don't, SplitView will assume you want it to fill the available space
   * and use RESIZE_AUTOMATIC.
   *
   * @default undefined
   *
   * @type Number
  */
  ...
  /**
   * [RO] The current position of the SplitView. This is read-only, and is set
  ...
  */
   ...
   *
   * For example, if SplitView positions this view at 100, and
   ...
   * @type Number
  */
  positionOffset: propertyFromRenderDelegate('splitPositionOffset', 0),

  /**
   * An amount to add to the assigned size when adjusting layout.
   *
   * For example, if SplitView gives this view a size of 1, but the
   ...
   * @type Number
  */
  sizeOffset: propertyFromRenderDelegate('splitSizeOffset', 0),

  /**
   * If true, the SplitView can collapse this view when the user
  ...
  */
   ...
   *
   * - RESIZE_AUTOMATIC: always resize when the SplitView changes size.
   * - RESIZE_MANUAL: resize only when the user moves a divider, or all
   *   RESIZE_AUTOMATIC views have already been resized as much as possible.
   * - FIXED_SIZE: Never resize.
   *
   * If you specify an autoResizeStyle, it will be used. If you leave it at `undefined`,
   * it will look at `size`: if a `size` is supplied, it will use `RESIZE_MANUAL`; otherwise
   * it will use `RESIZE_AUTOMATIC`.
   *
  ...
  */
  ...
  /**
   * If false, moving the divider before this view will not resize the view, but
   * instead, move it—causing further views to move as well. Most SplitView
   * children will want this set to true.
   *
   * @type Boolean
  */
  compensatesForMovement: true,

  /**
   * When false, the view can only be adjusted when the user drags a divider
   * immediately adjacent to it. If true, the view can be adjusted indirectly
   ...
   * set to true, SplitView will shrink the right pane to make way. If false, the
   ...
   * @type {Boolean}
  */
  allowsIndirectAdjustments: true,
  ...
  */
  splitChildLayoutDidChange: function() {
    ...
    if (split.get('layoutDirection') === LAYOUT_HORIZONTAL) {
      this.set('layout', {
        left: position,
        width: size,
        top: 0, bottom: 0
