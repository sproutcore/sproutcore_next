...
import { View, DROP_AFTER, DROP_BEFORE, DROP_ON, LAYOUT_HORIZONTAL, LAYOUT_VERTICAL, maxX, maxY, minX, minY } from "../../view/view.js";
import { CollectionRowDelegate } from "../mixins/collection_row_delegate.js";
import { DRAG_NONE } from "../system/drag.js";
import { CollectionView } from "./collection.js";
import { ListItemView } from "./list_item.js";
...
export const ListView = CollectionView.extend(CollectionRowDelegate,
/** @scope ListView.prototype */ {
  ...
  */
  ...
  /** @private CollectionView.prototype */
  ...
  */
  ...
  /** @private CollectionView.prototype.destroy. */
  ...
  */
  ...
  _sc_rowDelegateDidChange: function () {
    ...
    //@if(debug)
    if (!del) {
      throw new Error("%@ - Developer Error: ListView must always have a rowDelegate.".fmt(this));
    }
    //@endif
  ...
  */
  rowSizeForContentIndex: function (idx) {
    ...
    if (this.rowHeightForContentIndex) {
      //@if(debug)
      SC.warn('Developer Warning: The rowHeightForContentIndex() method of ListView has been renamed rowSizeForContentIndex().');
      //@endif
  ...
  /** @deprecated Version 1.11. Please use the `rowSizeDidChangeForIndexes()` function instead.
    ...
    @param {IndexSet|Number} indexes
    @returns {ListView} receiver
  */
  rowHeightDidChangeForIndexes: function (indexes) {
    //@if(debug)
    SC.warn('Developer Warning: The rowHeightDidChangeForIndexes() function of ListView has been renamed to rowSizeDidChangeForIndexes().');
    //@endif
  ...
  */
  ...
  /**
    ...
    @returns {Object} layout hash for the index provided
  */
  ...
  */
  ...
  /**
    ...
    @type View
  */
  insertionPointView: View.extend({
    classNames: 'sc-list-insertion-point',

    layout: function (key, value) {
      var layoutDirection = this.get('layoutDirection');

      key = layoutDirection === LAYOUT_HORIZONTAL ? 'width' : 'height';
    ...
    /**
      The direction of layout of the ListView.
  ...
  /**
    ...
    @see CollectionView#showInsertionPoint
  */
  showInsertionPoint: function (itemView, dropOperation) {
    ...
    if (this._sc_lastDropOnView) {
      this._sc_lastDropOnView.set('isDropTarget', false);
    ...
    if (layoutDirection === LAYOUT_HORIZONTAL) {
      ...
      if (dropOperation & DROP_AFTER) { left += itemViewLayout.width; }
      top = ((level + 1) * indent) + 12;
    } else {
      top = itemViewLayout.top;
      if (dropOperation & DROP_AFTER) { top += itemViewLayout.height; }
  ...
  /**
    ...
    value is an index plus a dropOperation, which is computed as such:
      ...
      - if dropOperation = DROP_ON and you are over the middle of a row, then
        use DROP_ON.

    @see CollectionView.insertionIndexForLocation
  */
  insertionIndexForLocation: function (loc, dropOperation) {
    ...
    if (dropOperation === DROP_BEFORE) {
      ...
      if (index < 0) dropOperation = DRAG_NONE;
    }
