...
/** @class
  ...
  and `horizontalScrollerView`). `ScrollerView` is a gutter-based scroller which persists and
  takes up fourteen pixels. (By default, no scroller is shown for content that is too small to
  scroll; see `autohidesHorizontalScroller` and `autohidesVerticalScroller`.) `OverlayScrollerView`
...
/** @scope ScrollView.prototype */
  ...
  /** @see View.prototype.acceptsMultitouch
  ...
  */
  ...
  animationCurveDecelerate: easingCurve(0.35,0.34,0.84,1), // 'cubic-bezier(.35,.34,.84,1)',  // http://cubic-bezier.com

  /** @private Animation curves. Kept private b/c it will likely become a computed property. */
  animationCurveReverse: easingCurve(0.45,-0.47,0.73,1.3), // 'cubic-bezier(0.45,-0.47,0.73,1.3)',

  /** @private Animation curves. Kept private b/c it will likely become a computed property. */
  animationCurveSnap: easingCurve(0.28,0.36,0.52,1), // 'cubic-bezier(.28,.36,.52,1)',
  ...
  */
  ...
  /**
    ...
    @type Number
    @default SCROLL.DRAG_SCROLL_DECELERATION
  */
  decelerationRate: SCROLL.DRAG_SCROLL_DECELERATION,
  ...
  /** @private
    ...
    If you set this to false, the touch will not trigger a scroll until you pass control back to this view via
  ...
  */
  ...
  /**
    ...
    If `horizontalOverlay` is `true`, the default view used will be an OverlayScrollerView,
    otherwise ScrollerView will be used.

    @type View
    @default ScrollerView | OverlayScrollerView
  */
  ...
  /**
    ...
    @default ALIGN_LEFT
  */
  initialHorizontalAlign: outlet('horizontalAlign'),
  ...
  /**
    ...
    @default ALIGN_TOP
  */
  initialVerticalAlign: outlet('verticalAlign'),
  ...
  */
  ...
  /**
    ...
    If the contentView implements the Scalable protocol, it will instead pass the scale to the contentView's
  ...
  */
  ...
  /**
    ...
    You can change this value for all instances of ScrollView in your application by overriding
    `SCROLL.SCALE_GESTURE_THRESHOLD` at launch time.

    @type Number
    @default SCROLL.SCALE_GESTURE_THRESHOLD
  */
  scaleGestureThreshold: SCROLL.SCALE_GESTURE_THRESHOLD,

  /**
    This determines how far (in pixels) a gesture must move before it is registered as a scroll.

    You can change this value for all instances of ScrollView in your application by overriding
    `SCROLL.SCROLL_THRESHOLD` at launch time.

    @type Number
    @default SCROLL.SCROLL_GESTURE_THRESHOLD
  */
  scrollGestureThreshold: SCROLL.SCROLL_GESTURE_THRESHOLD,
  ...
  /**
    ...
    You can change this value for all instances of ScrollView in your application by overriding
    `SCROLL.SCROLL_LOCK_GESTURE_THRESHOLD` at launch time.

    @type Number
    @default SCROLL.SCROLL_LOCK_GESTURE_THRESHOLD
  */
  scrollLockGestureThreshold: SCROLL.SCROLL_LOCK_GESTURE_THRESHOLD,
  ...
  /** @private
    ...
    You can change this value for all instances of ScrollView in your application by overriding
    `SCROLL.TOUCH.DEFAULT_SECONDARY_SCROLL_THRESHOLD` at launch time.

    @type Number
    @default SCROLL.TOUCH.DEFAULT_SECONDARY_SCROLL_THRESHOLD
  */
  // scrollSecondaryGestureThreshold: SCROLL.TOUCH.DEFAULT_SECONDARY_SCROLL_THRESHOLD,
  ...
  */
  ...
  /**
    ...
    If `verticalOverlay` is `true`, the default view used will be an OverlayScrollerView,
    otherwise ScrollerView will be used.
  ...
  */
  ...
  _sc_cancelAnimation: function () {
    ...
    // UNUSED. Animate using View.prototype.animate. Cancelling the animation in place proved problematic.
    // if (contentView.get('viewState') === CoreView.ATTACHED_SHOWN_ANIMATING) {
    //   // Stop the animation in place.
    //   contentView.cancelAnimation(LayoutState.CURRENT);
  ...
  */
  _sc_maximumScrollOffset: function (contentSize, containerSize, align, canScroll) {
    ...
    if (canScroll === false) contentSize = Math.min(contentSize, containerSize);
    ...
    if (align === ALIGN_LEFT || align === ALIGN_TOP) {
      ...
      return 0;
    } else if (align === ALIGN_MIDDLE || align === ALIGN_CENTER) {
  ...
  */
  ...
  /** @private Registers/deregisters view with Drag for autoscrolling. */
  _sc_registerAutoscroll: function () {
    if (this.get('isVisibleInWindow') && this.get('isEnabledInPane')) {
      Drag.addScrollableView(this);
    } else {
      Drag.removeScrollableView(this);
    }
  },
  ...
  _sc_repositionContentViewUnfiltered: function () {
    ...
    // Otherwise call adjust on the content.
      ...
      var contentAdjustMap = ScrollView._SC_CONTENT_ADJUST_MAP; // Shared object used to avoid continually initializing/destroying objects.

      // Create the content adjust map once. Note: This is a shared object, all properties must be overwritten each time.
      if (!contentAdjustMap) {
        contentAdjustMap = ScrollView._SC_CONTENT_ADJUST_MAP = {
  ...
  _sc_repositionScrollersUnfiltered: function () {
    var hasHorizontalScroller = this.get('hasHorizontalScroller'),
      ...
      containerLayoutMap = ScrollView._SC_CONTAINER_LAYOUT_MAP, // Shared object used to avoid continually initializing/destroying objects.
    ...
    if (!containerLayoutMap) { containerLayoutMap = ScrollView._SC_CONTAINER_LAYOUT_MAP = {}; }
  ...
  _sc_scaleDidChange: function () {
    ...
    if (contentView) {
      if (contentView.useStaticLayout) {
        ...
        if (scale !== 1) {
          warn("Developer Warning: ScrollView's `scale` feature does not support statically laid out content views.");
        }
        //@endif
  ...
  createChildViews: function () {
    ...
    if (!this.get('hasHorizontalScroller')) {
      ...
      if (scrollerView == null) {
        scrollerView = this.get('horizontalOverlay') ? OverlayScrollerView : ScrollerView;
      }
    ...
    if (!this.get('hasVerticalScroller')) {
      ...
      /* jshint eqnull:true */
      if (scrollerView == null) {
        scrollerView = this.get('verticalOverlay') ? OverlayScrollerView : ScrollerView;
      }
  ...
  didCreateLayer: function () {
    ...
    // Observe for changes in enablement and visibility for registering with Drag auto-scrolling and initialize once.
  ...
  */
  scrollBy: function (x, y) {
    ...
    if (y === undefined && SC.typeOf(x) === SC.T_HASH) {
      ...
      SC.warn("Developer Warning: Passing an object to ScrollView.scrollBy is deprecated. Please pass both the x and y arguments.");
      //@endif
  ...
  */
  scrollTo: function (x, y) {
    ...
    if (y === undefined && SC.typeOf(x) === SC.T_HASH) {
      ...
      // Add some developer support. It's faster to pass the arguments individually so that we don't need to do this normalization and the
      // developer isn't creating an extra Object needlessly.
      SC.warn("Developer Warning: Passing an object to ScrollView.scrollTo is deprecated. Please pass both the x and y arguments.");
      //@endif
  ...
  */
  scrollToRect: function (rect) {
    ...
    var vo = cloneRect(this.get('containerView').get('frame')),
    ...
    vo.y += Math.max(0, maxY(rect) - maxY(vo));
    vo.x += Math.max(0, maxX(rect) - maxX(vo));

    // if top edge is not visible, shift origin
    vo.y -= Math.max(0, minY(vo) - minY(rect));
    vo.x -= Math.max(0, minX(vo) - minX(rect));
  ...
  */
  ...
  /**
    ...
    @param {View} view view to scroll or null to scroll receiver visible
    @returns {Boolean} true if scroll position was changed
  */
  scrollToVisible: function scrollToVisible (view) {
    ...
    if (!viewFrame) return false; // nothing to do
  ...
  willDestroyLayer: function () {
    ...
    // Be sure to remove this view as a scrollable view for Drag.
  ...
  */
        ...
        if (this._sc_isTouchScrollingH) {
          ...
          // Slide.
            ...
            if (horizontalScrollOffset > maximumHorizontalScrollOffset) {
              ...
              c2y = 2 * c2x;
              this._sc_animationTiming = easingCurve(0.0,0.5,c2x.toFixed(1),c2y.toFixed(1)); // 'cubic-bezier(0.0,0.5,%@,%@)'.fmt(c2x.toFixed(1), c2y.toFixed(1));
  ...
  /** @private @see RootResponder.prototype.captureTouch */
    ...
    if (canScale) {
      ...
      if (absDeltaD > 0) {
        ...
        if (!this._sc_containerOffset) {
          ...
          this._sc_containerOffset = offset(el);
          this._sc_touchCenterX = avgTouch.x - this._sc_containerOffset.x;
          this._sc_touchCenterY = avgTouch.y - this._sc_containerOffset.y;
        }
