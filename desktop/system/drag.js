...
/**
  @static
*/
export const DRAG_LINK = 0x0004;

/**
  @static
*/
export const DRAG_COPY = 0x0001;

/**
  @static
*/
export const DRAG_MOVE = 0x0002;

/**
  @static
*/
export const DRAG_NONE = 0x0000;

/**
  @static
*/
export const DRAG_ANY = 0x000F;

/**
  @static
*/
export const DRAG_DATA = 0x0008; // includes DRAG_REORDER

/**
  @static
*/
export const DRAG_AUTOSCROLL_ZONE_THICKNESS = 20;
...
/**
  ...
  To initiate a drag, you should call `Drag.start()` with the options below
  specified in a hash. Pass the ones you need to get the drag you want:
    ...
    - `dataSource` --  Optional object that will provide the data for the drag to
      ...
      DragDataSourceProtocol protocol.
...
*/
export const Drag = SC.Object.extend(
/** @scope Drag.prototype */ {
  ...
  */
  ...
  /**
    If `false`, the source will not be copied, clone, no ghost view will get created,
    and it won't be moved.

    @type Boolean
  */
  sourceIsDraggable: true,
  ...
  */
  ...
  /**
    ...
    Data Source objects must comply with the `DragDataSourceProtocol` interface.  If
  ...
  */
  ...
  /**
    ...
    @type Number
  */
  allowedDragOperations: DRAG_ANY,
  ...
  */
  ...
  /** @private
    This will actually start the drag process. Called by Drag.start().
  */
  startDrag: function () {
    ...
    if (this.get('sourceIsDraggable')) {
      ...
      if (evt.makeTouchResponder) {
        ...
        SC.Timer.schedule({
          target: evt,
          action: function () {
            if (!evt.hasEnded) evt.makeTouchResponder(self, true);
          },
          interval: 1
        });
      }
  ...
  */
  ...
  /** @private
    ...
    @param {DragOp} op The drag operation that was performed. One of
      DRAG_COPY, DRAG_MOVE, DRAG_LINK, or DRAG_NONE.
  */
  endDrag: function (evt, op) {
    ...
    for (var idx = 0, len = ary.length; idx < len; idx++) {
      try {
        ...
        SC.Logger.error('Exception in Drag.mouseUp(dragEnded on %@): %@'.fmt(ary[idx], ex2));
      }
    }

    // Trigger a slide-back if triggered and if the drag was unsuccessful.
    if (this.get('sourceIsDraggable') && this.get('slideBack') && op === DRAG_NONE) {
    ...
    if (source) {
      ...
      if (source.dragDidSucceed && op !== DRAG_NONE) source.dragDidSucceed(this, loc, op);
      // notify the source that the drag was cancelled
      else if (source.dragDidCancel && op === DRAG_NONE) source.dragDidCancel(this, loc, op);
  ...
  */
  mouseDragged: function (evt) {
    ...
    while (target && (target !== last) && (op === DRAG_NONE)) {
      ...
      } else op = DRAG_ANY; // assume drops are allowed

      // now, let's see if the target will accept the drag
      if ((op !== DRAG_NONE) && target && target.computeDragOperations) {
        op = op & target.computeDragOperations(this, evt, op);
      } else op = DRAG_NONE; // assume drops AREN'T allowed

      this.allowedDragOperations = op;

      // if DRAG_NONE, then look for the next parent that is a drop zone
      if (op === DRAG_NONE) target = this._findNextDropTarget(target);
    }
  ...
  mouseUp: function (evt) {
    ...
    try {
      if (target && target.acceptDragOperation && target.acceptDragOperation(this, op)) {
        op = target.performDragOperation ? target.performDragOperation(this, op) : DRAG_NONE;
      } else {
        op = DRAG_NONE;
      }
    } catch (e) {
      SC.Logger.error('Exception in Drag.mouseUp(acceptDragOperation|performDragOperation): %@'.fmt(e));
    }
  ...
  */
  _createGhostView: function () {
    ...
    view = this.ghostView = Pane.create({
  ...
  */
  ...
  /** @private Called instead of _destroyGhostView if slideBack is true. */
  ...
  */
  _dropTargets: function () {
    ...
    var depth = {};
    var dropTargets = Drag._dropTargets;
  ...
  */
  _findDropTarget: function (evt) {
    ...
    for (var idx = 0, len = ary.length; idx < len; idx++) {
      ...
      if (pointInRect(loc, frame)) return target;
    }
    return null;
  },
  ...
  _autoscroll: function (evt) {
    ...
    while (view && !scrollableView) {
      ...
      if (vscroll) {
        ...
        // bottom hotzone?
        max = maxY(f);
        min = max - DRAG_AUTOSCROLL_ZONE_THICKNESS;
        if (loc.y >= min && loc.y <= max) vscroll = 1;
        else {
          // how about top
          min = minY(f);
          max = min + DRAG_AUTOSCROLL_ZONE_THICKNESS;
          if (loc.y >= min && loc.y <= max) vscroll = -1;
  ...
  */
  _scrollableViews: function () {
    ...
    var ret = [];
    var hash = Drag._scrollableViews;
    for (var key in hash) {
      if (hash.hasOwnProperty(key)) ret.push(hash[key]);
    }
  ...
  */
  _findNextScrollableView: function (view) {
    var scrollableViews = Drag._scrollableViews;
    while ((view = view.get('parentView'))) {
...
/** @scope Drag */ {
  ...
  /**
   ...
   @params {Object} ops a hash of options.  See documentation above.
  */
  start: function (ops) {
  ...
  /**
    ...
    @param {View} target a view implementing the DropTargetProtocol protocol
  */
  ...
  /**
    ...
    @param {View} target A previously registered drop target
  */
  ...
  /**
    ...
    @param {View} target The view that should be auto-scrolled
  */
  ...
  /**
    ...
    @param {View} target A previously registered scrollable view
  */
...
View.reopen(
  ...
  init: function (original) {
    ...
    // register for drags
    if (this.get('isDropTarget')) { Drag.addDropTarget(this); }

    // register scroll views for autoscroll during drags
    if (this.get('isScrollable')) { Drag.addScrollableView(this); }
  }.enhance(),

  /** @private */
  destroy: function (original) {
    // unregister for drags
    if (this.get('isDropTarget')) { Drag.removeDropTarget(this); }

    // unregister for autoscroll during drags
    if (this.get('isScrollable')) { Drag.removeScrollableView(this); }
